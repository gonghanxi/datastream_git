...
bool ComplexExpGen_Block::Initialize()
{
    ...
    try { m_offset = ParseComplex(getParameter("Offset").Value); } catch (...) { LOG_WARN("Failed to parse parameter 'Offset', using default value."); }
    ...
    try { m_quadraturePolarity = ConvertStringToQuadraturePolarity(getParameter("QuadraturePolarity").Value); } catch (...) { LOG_WARN("Failed to parse parameter 'QuadraturePolarity', using default value."); }
