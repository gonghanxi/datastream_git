#include "RADAR_Pf_Measurement_Block.h"
#include <QDir>

RADAR_Pf_Measurement_Block::RADAR_Pf_Measurement_Block(const std::string &name)
    :Block(name)
{
    FalseCount = 0;
}

bool RADAR_Pf_Measurement_Block::Setup()
{
   Block::Setup();
...
bool RADAR_Pf_Measurement_Block::Initialize()
{
     SetBlockType(Block::BlockType::SINK);
     m_radar = std::make_unique<RADAR_Pf_Measurement>();
...
bool RADAR_Pf_Measurement_Block::Run()
{
...
bool RADAR_Pf_Measurement_Block::Done()
{
    // 如果文件已经打开，直接写入剩余数据
    if (m_qfile.isOpen()) {
        // 文件已打开，跳过打开文件的步骤
    ...
    m_stream << "\t{" << "\r\n";
    m_stream << "\t\t" << R"("Index": )" << 1 << "," << "\r\n";
    m_stream << "\t\t" << R"("Pf":)" << 1.0*FalseCount / (Stop - Start) << "\r\n";
    m_stream << "\t}" /*<< "\r\n"*/;
...
void RADAR_Pf_Measurement_Block::SetParameters()
{
    if(!m_radar) return;
    m_radar->Start = Start;
    m_radar->Stop = Stop;
...
void RADAR_Pf_Measurement_Block::SetDefaultParameters()
{
    Start = 0;
    Stop = 1000;
}

void RADAR_Pf_Measurement_Block::cleanup()
{
    // 确保文件关闭
    if (m_qfile.isOpen()) {
        m_qfile.close();
...
bool RADAR_Pf_Measurement_Block::openFileForAppend()
{
    QString filePath = QString::fromUtf8(FileName);
...
bool RADAR_Pf_Measurement_Block::ModelSetup()
{
    bool bStatus = true;
    ...
    if (Stop <= Start)
    {
        LOG_ERROR("Stop must be > Start.");
        bStatus = false;
    }
    m_control.Initialize(nullptr, Start, Stop);
    return bStatus;
}
