...
bool SinkInt_M_Block::Setup()
{
    ...
    QString folderPath = outputPath + "/07";
...
bool SinkInt_M_Block::Run()
{
    ...
    // 处理变长数据（TIMED_INT_MATRIX）或普通 DOUBLE
    // 读取一个或多个数据点
    // 对于每个数据点：
...
{
    qDebug() << "[SinkInt_M_Block] Done - 处理剩余数据"
...
bool SinkInt_M_Block::IsCollectionComplete()
{
...
// ---------- 文件操作 ----------
bool SinkInt_M_Block::openFileForWrite()
{
    m_qfile.setFileName(m_fullPath);
    if (!m_qfile.open(QIODevice::WriteOnly | QIODevice::Text)) {
        LOG_INFO("[SinkInt_M_Block] 无法创建文件:", m_qfile.errorString().toStdString());
        return false;
    }
    ...
    qDebug() << "[SinkInt_M_Block] 文件已创建:" << m_fullPath;
    return true;
}
...
void SinkInt_M_Block::closeFileProperly()
{
...
void SinkInt_M_Block::writeDataPointToStream(size_t bufferIndex, unsigned long long dataIndex)
{
...
// ---------- 时间驱动中途刷新 ----------
void SinkInt_M_Block::flushToFile()
{
...
bool SinkInt_M_Block::isTimeDrivenMode() const { return m_isTimeDrivenMode; }
void SinkInt_M_Block::setTimeDrivenMode(bool enabled) { m_isTimeDrivenMode = enabled; }
double SinkInt_M_Block::GetCurrentSimulationTime() const { return m_currentSimulationTime; }
