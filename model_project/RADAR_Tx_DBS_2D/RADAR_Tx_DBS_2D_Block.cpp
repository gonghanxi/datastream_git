#include "RADAR_Tx_DBS_2D_Block.h"
...
RADAR_Tx_DBS_2D_Block::RADAR_Tx_DBS_2D_Block(const std::string& name)
    : Block(name)
{
}
...
bool RADAR_Tx_DBS_2D_Block::Setup()
{
    Block::Setup();
    return true;
}
...
bool RADAR_Tx_DBS_2D_Block::Run()
{
...
bool RADAR_Tx_DBS_2D_Block::DataStreamRun()
{
    ...
    for (int idx = 0; idx < nChExpected; ++idx)
    {
        // row-major / x-fast: idx = n * Nx + m
        const int n = idx / m_nx;
        const int m = idx % m_nx;

        const double psi = kTwoPi * (m_xPos[m] * kx + m_yPos[n] * ky);
        const Cx phase(std::cos(psi), -std::sin(psi)); // exp(-j*psi)

        outputData.push_back(xin * (m_taper2d[idx] * phase));
    }
...
bool RADAR_Tx_DBS_2D_Block::TimeDrivenRun()
{
    using Cx = std::complex<double>;
    ...
    if (static_cast<int>(m_inputBuffer.size()) >= 1)
    {
        ...
        for (int idx = 0; idx < m_nChExpected; ++idx)
        {
            const int n = idx / m_nx;
            const int m = idx % m_nx;
            const double psi = kTwoPi * (m_xPos[m] * kx + m_yPos[n] * ky);
            const Cx phase(std::cos(psi), -std::sin(psi));
            m_outputQueue.push(xin * (m_taper2d[idx] * phase));
        }
...
bool RADAR_Tx_DBS_2D_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_algo = std::make_unique<RADAR_Tx_DBS_2D>();
    ...
    {
        LOG_ERROR("RADAR_Tx_DBS_2D: NumOfAntx and NumOfAnty must be >= 1.");
        return false;
    }
...
RADAR_Tx_DBS_2D::Window_TypeEnum RADAR_Tx_DBS_2D_Block::ConvertStringToWindowType(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "rectangle"      || lower == "0") return RADAR_Tx_DBS_2D::Rectangle;
    if (lower == "bartlett"       || lower == "1") return RADAR_Tx_DBS_2D::Bartlett;
    if (lower == "hanning"        || lower == "2") return RADAR_Tx_DBS_2D::Hanning;
    if (lower == "hamming"        || lower == "3") return RADAR_Tx_DBS_2D::Hamming;
    if (lower == "blackman"       || lower == "4") return RADAR_Tx_DBS_2D::Blackman;
    if (lower == "steepblackman"  || lower == "5") return RADAR_Tx_DBS_2D::SteepBlackman;
    if (lower == "kaiser"         || lower == "6") return RADAR_Tx_DBS_2D::Kaiser;
    return RADAR_Tx_DBS_2D::Rectangle;
}
...
void RADAR_Tx_DBS_2D_Block::rebuildCache()
{
    // 阵元位置（corner-origin）
...
double RADAR_Tx_DBS_2D_Block::deg2rad(double deg)
{
...
double RADAR_Tx_DBS_2D_Block::i0Bessel(double x)
{
...
void RADAR_Tx_DBS_2D_Block::makeWindow(RADAR_Tx_DBS_2D::Window_TypeEnum type, int L, double beta, std::vector<double>& w)
{
    ...
    switch (type)
    {
    case RADAR_Tx_DBS_2D::Rectangle:
        for (int p = 0; p < L; ++p) w[p] = 1.0;
        break;

    case RADAR_Tx_DBS_2D::Bartlett:
        for (int p = 0; p < L; ++p) {
    ...
    case RADAR_Tx_DBS_2D::SteepBlackman: {
