#include "BSF_ChebyshevII_Block.h"
#include <algorithm>
#include <cctype>
#include <cmath>
#include <complex>
...
BSF_ChebyshevII_Block::BSF_ChebyshevII_Block(const std::string& name)
    : Block(name)
{
}

void BSF_ChebyshevII_Block::SetDefaultParamters()
{
    m_loss = 0.0;
    m_fCenter = 150e3;
    ...
    m_stopRipple = 50.0;
    m_orderType = BSF_ChebyshevII::UserDefined;
    m_order = 5;
    m_sampleRate = getSimu().samplingRate;
}

void BSF_ChebyshevII_Block::SetParameters()
{
    if (!m_bsf) {
        return;
    }
    ...
    m_bsf->PassAtten = m_passAtten;
    m_bsf->StopBandwidth = m_stopBandwidth;
    m_bsf->StopRipple = m_stopRipple;
    m_bsf->OrderType = m_orderType;
    m_bsf->Order = m_order;
    m_bsf->SampleRate = m_sampleRate;
}

bool BSF_ChebyshevII_Block::Setup()
{
    Block::Setup();
    return true;
}

bool BSF_ChebyshevII_Block::Run()
{
...
bool BSF_ChebyshevII_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_bsf = std::make_unique<BSF_ChebyshevII>();
...
void BSF_ChebyshevII_Block::UpdateCharacterizationFrequency()
{
...
BSF_ChebyshevII::SelectedOrderType BSF_ChebyshevII_Block::ConvertStringToOrderType(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "userdefined") {
        return BSF_ChebyshevII::UserDefined;
    }
    if (lower == "auto" || lower == "0") {
        return BSF_ChebyshevII::Auto;
    }
    return BSF_ChebyshevII::UserDefined;
}
