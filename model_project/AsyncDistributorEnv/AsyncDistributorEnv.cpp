...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE(AsyncDistributorEnv)
{
	SET_MODEL_DESCRIPTION("Asynchronous Data Distributor (Envelope)");
...

	{
		...
		p.SetName("input");
		p.SetDescription("input envelope data stream");
	}

	{
		DFPort p = ADD_MODEL_OUTPUT(output);
		p.SetName("output");
		p.SetDescription("bus of output envelope data streams");
	}
...
AsyncDistributorEnv::AsyncDistributorEnv()
	: BlockSizes(1, 1)
{
	BlockSizes(0) = 1;
}

bool AsyncDistributorEnv::Setup()
{
	...
	if (numBlockVals == 0)
	{
		POST_ERROR("AsyncDistributorEnv: BlockSizes must contain at least one element.");
		return false;
	}
	...
	if (numBlockVals == 1)
	{
		...
		if (B <= 0)
		{
			POST_ERROR("AsyncDistributorEnv: elements of BlockSizes array must all be > 0.");
			return false;
		}
		...
		std::stringstream ss;
		ss << "AsyncDistributorEnv: BlockSizes has 1 element, applying this value to all "
			<< numOutputs << " output(s).";
	...
	else if (numBlockVals >= numOutputs)
	{
		...
		{
			std::stringstream ss;
			ss << "AsyncDistributorEnv: BlockSizes has "
	...
	else 
	{
		...
		std::stringstream ss;
		ss << "AsyncDistributorEnv: BlockSizes has " << numBlockVals
	...
	for (size_t i = 0; i < numOutputs; ++i)
	{
		...
		if (Bi <= 0)
		{
			POST_ERROR("AsyncDistributorEnv: expanded BlockSizes element is not > 0.");
			return false;
		}
	...
	if (numInputData == 0)
	{
		POST_ERROR("AsyncDistributorEnv: total number of input samples per run must be > 0.");
		return false;
	}
