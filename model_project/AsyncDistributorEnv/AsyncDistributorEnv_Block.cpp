#include "AsyncDistributorEnv_Block.h"

AsyncDistributorEnv_Block::AsyncDistributorEnv_Block(const std::string &name)
    :Block(name)
{

}
bool AsyncDistributorEnv_Block::Setup()
{
    Block::Setup();
...
bool AsyncDistributorEnv_Block::Run()
{
    if(IsVariableStepMode()) return TimeDrivenRun();
    return DataStreamRun();
}

bool AsyncDistributorEnv_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_AsyncDistributor = std::make_unique<AsyncDistributorEnv>();
    ...
    AddInputPort("input", m_AsyncDistributor->input, 1, DataType::ENVELOPE_SIGNAL);
    AddOutputPort("output", m_AsyncDistributor->output, 1, DataType::ENVELOPE_BUS);
    return true;
}

void AsyncDistributorEnv_Block::SetParameters()
{
    if(!m_AsyncDistributor) return;
    m_AsyncDistributor->BlockSizes = m_BlockSizes;
}

void AsyncDistributorEnv_Block::SetDefaultParameters()
{
    m_BlockSizes.Resize(1,1);
    m_BlockSizes(0,0) = 1;
}

bool AsyncDistributorEnv_Block::ModelsSetup()
{
...
bool AsyncDistributorEnv_Block::DataStreamRun()
{
    std::string inputPort = GetInputPortName(0);
    std::string outputPort = GetOutputPortName(0);

    // 读取所有输入数据
    std::vector<EnvelopeSignal> inputData = ReadInputData<EnvelopeSignal>(inputPort);
    ...
    for (size_t i = 0; i < numOutputs; ++i) {
        ...
        // 提取当前通道的数据块
        std::vector<EnvelopeSignal> channelData;
...
bool AsyncDistributorEnv_Block::TimeDrivenRun()
{
    std::string inputPort = GetInputPortName(0);
    std::string outputPort = GetOutputPortName(0);
    ...
    for (size_t i = 0; i < m_channelQueues.size(); ++i) {
        if (!m_channelQueues[i].empty()) {
            EnvelopeSignal sample = m_channelQueues[i].front();
            m_channelQueues[i].pop();
            // 写入单个样本到对应通道
            outputBuffer->WriteDataToChannel(static_cast<int>(i), std::vector<EnvelopeSignal>{sample});
            anyOutput = true;
