#include "TransposeInt_Block.h"

TransposeInt_Block::TransposeInt_Block(const std::string &name)
    :Block(name)
{
...
bool TransposeInt_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);
    m_Transpose = std::make_unique<TransposeInt>();
    ...
    AddInputPort("input", m_Transpose->input, static_cast<size_t>(SamplesInRow * NumberOfRows), DataType::CIRCULAR_BUFFER_INT);
    AddOutputPort("output", m_Transpose->output, static_cast<size_t>(SamplesInRow * NumberOfRows), DataType::CIRCULAR_BUFFER_INT);
    return true;
}
...
void TransposeInt_Block::SetDefaultParameters()
{
    SamplesInRow = 8;
    NumberOfRows = 8;
}

bool TransposeInt_Block::ModelSetup()
{
