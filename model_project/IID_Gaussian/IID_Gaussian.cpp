...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE ( IID_Gaussian )
{	
	SET_MODEL_DESCRIPTION("IID Gaussian Noise Waveform");
	...
	{
		SystemVueModelBuilder::DFParam param = ADD_MODEL_PARAMETER(StdDev);
		param.SetDescription("Standard deviation");
...

	{
		...
		param.SetDescription("Offset value");
		param.SetUnit(SystemVueModelBuilder::Units::VOLTAGE);
...
bool IID_Gaussian::Run()
{
	...
	std::normal_distribution<double>	dN(Offset, StdDev);
	output[0] = dN(gen);
	...
	// 单脉冲模式输出
	if (BurstMode == IID_Gaussian::Single)
	{
	...
	// 多脉冲模式疏输出
	if (BurstMode == IID_Gaussian::Multiple)
	{
