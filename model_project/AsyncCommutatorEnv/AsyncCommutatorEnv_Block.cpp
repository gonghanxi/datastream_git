#include "AsyncCommutatorEnv_Block.h"
...
namespace {
bool FcEqual(double a, double b)
{
...
AsyncCommutatorEnv_Block::AsyncCommutatorEnv_Block(const std::string& name)
    : Block(name)
    , m_blockSizes(1, 1)
{
    m_blockSizes(0) = 1;
...
void AsyncCommutatorEnv_Block::SetDefaultParamters()
{
    m_blockSizes.Resize(1, 1);
    m_blockSizes(0) = 1;
}

void AsyncCommutatorEnv_Block::SetParameters()
{
    if (!m_asyncCommutatorEnv) {
        return;
    }

    m_asyncCommutatorEnv->BlockSizes = m_blockSizes;
}

bool AsyncCommutatorEnv_Block::Setup()
{
    Block::Setup();
...
bool AsyncCommutatorEnv_Block::Run()
{
    if(IsVariableStepMode()) return TimeDrivenRun();
    return DataStreamRun();
}

bool AsyncCommutatorEnv_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_asyncCommutatorEnv = std::make_unique<AsyncCommutatorEnv>();

    AddInputPort("input", m_asyncCommutatorEnv->input, 1, Block::DataType::ENVELOPE_BUS);
    AddOutputPort("output", m_asyncCommutatorEnv->output, 1, Block::DataType::ENVELOPE_SIGNAL);
...
bool AsyncCommutatorEnv_Block::UpdateCharacterizationFrequency()
{
    ...
    const size_t numInputs = inputPort->GetBusConnectionCount();
    ...
    for (size_t i = 0; i < numInputs; ++i) {
        ...
        const double fcIn = inputPort->GetBusConnections().at(static_cast<int>(i)).bridgeReader->getCharacterizationFrequency();
        if (!fcInitialized) {
            fcOut = fcIn;
            fcInitialized = true;
        } else if (!FcEqual(fcIn, fcOut)) {
            std::stringstream ss;
            ss << "AsyncCommutatorEnv: characterization frequency mismatch for input#" << (i + 1)
               << " fc=" << fcIn << " differs from output fc=" << fcOut;
    ...
    if (fcInitialized) {
        outputPort->setCharacterizationFrequency(fcOut);
    }
    return true;
}

bool AsyncCommutatorEnv_Block::ModelSetup()
{
    auto* inputPort = GetInputPort(GetInputPortName(0));
    auto* outputPort = GetOutputPort(GetOutputPortName(0));
...
bool AsyncCommutatorEnv_Block::DataStreamRun()
{
    const size_t numBlockSizes = static_cast<size_t>(m_blockSizes.NumElements());
    ...
    if (!UpdateCharacterizationFrequency()) {
        return false;
    }
...
bool AsyncCommutatorEnv_Block::TimeDrivenRun()
{
    ...
    if (CanProcessData) {
        ...
        std::vector<EnvelopeSignal> outputBlock;
        ...
        for (EnvelopeSignal val : outputBlock) {
    ...
    if (!m_outputQueue.empty()) {
        ...
        qDebug() << "[AsyncCommutator] no data";
    }
