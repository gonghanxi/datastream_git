...
bool Ramp_Block::Initialize()
{
    ...
    try { m_stepPerSample = std::stod(getParameter("StepPerSample").Value); } catch (...) { LOG_WARN("Failed to parse parameter 'StepPerSample', using default value."); }
    try { m_initialValue = std::stod(getParameter("InitialValue").Value); } catch (...) { LOG_WARN("Failed to parse parameter 'InitialValue', using default value."); }
