...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE ( Ramp )
{	
	SET_MODEL_DESCRIPTION("Ramp Waveform");
	SET_MODEL_SYMBOL("SYM_Ramp");
	...
	{
		SystemVueModelBuilder::DFParam param = ADD_MODEL_PARAMETER(StepPerSample);
		param.SetDescription("Step value per simulation sample");
		param.SetUnit(SystemVueModelBuilder::Units::NONE);
		param.SetDefaultValue("1");
	}

	{
		SystemVueModelBuilder::DFParam param = ADD_MODEL_PARAMETER(InitialValue);
		param.SetDescription("Initial ramp voltage value");
		param.SetUnit(SystemVueModelBuilder::Units::NONE);
...
bool Ramp::Run()
{
	...
	else if (i == InitialDelay)
	{
		output[0] = InitialValue;
	}
	else
	{
		output[0] += StepPerSample;
	}
	return true;
}
