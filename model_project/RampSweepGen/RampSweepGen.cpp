...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE ( RampSweepGen )
{	
	SET_MODEL_DESCRIPTION("Ramp Sweep Waveform Generator");
	SET_MODEL_SYMBOL("SYM_RampSweepGen");
...
RampSweepGen::RampSweepGen()
{
	
}

bool RampSweepGen::Setup()
{
	bool bStatus = true;
...
bool RampSweepGen::Run()
{
	...
	else
	{
		...
		double tInflection = Symmetry / 100 / Fsweep;
		...
		{
			output[0] = Polarity ? 
				LoLevel + (HiLevel - LoLevel)*(pt - tInflection) / (1 / Fsweep - tInflection) :
				HiLevel + (LoLevel - HiLevel)*(pt - tInflection) / (1 / Fsweep - tInflection);
		}
