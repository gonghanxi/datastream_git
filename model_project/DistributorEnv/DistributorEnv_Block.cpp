#include "DistributorEnv_Block.h"

DistributorEnv_Block::DistributorEnv_Block(const std::string &name)
    :Block(name)
{

}
bool DistributorEnv_Block::Setup()
{
    Block::Setup();
...
bool DistributorEnv_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_Distributor = std::make_unique<DistributorEnv>();
    ...
    AddInputPort("input", m_Distributor->input, 1, DataType::ENVELOPE_SIGNAL);
    AddOutputPort("output", m_Distributor->output, 1, DataType::ENVELOPE_BUS);
    return true;
}

void DistributorEnv_Block::SetParameters()
{
    if(!m_Distributor) return;
    m_Distributor->BlockSize = m_BlockSize;
}

void DistributorEnv_Block::SetDefaultParameters()
{
    m_BlockSize = 1;
}

bool DistributorEnv_Block::ModelsSetup()
{
