...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE ( RampGen )
{	
	SET_MODEL_DESCRIPTION("Ramp Waveform Generator");
	SET_MODEL_SYMBOL("SYM_RampGen");
...

	{
		...
		param.SetDescription("Waveform frequency");
	...
	{
		SystemVueModelBuilder::DFParam param = ADD_MODEL_PARAMETER(Symmetry);
		param.SetDescription("Symmetry in percent");
...
bool RampGen::Setup()
{
	...
	{
		POST_ERROR("Symmetry must be >= 0 and <= 100.");
		bStatus = false;
	}
...
bool RampGen::Run()
{
	...
	double tInflection = Symmetry / 100 / Frequency;
	...
	case RampGen::OFF:
	{
		...
		{
			output[0] = Polarity ? 
				HiLevel + (LoLevel - HiLevel)*pt / tInflection : 
				LoLevel + (HiLevel - LoLevel)*pt / tInflection;
		}
		else
		{
			output[0] = Polarity ?
				LoLevel + (HiLevel - LoLevel)*(pt - tInflection) / (1 / Frequency - tInflection) : 
				HiLevel + (LoLevel - HiLevel)*(pt - tInflection) / (1 / Frequency - tInflection);
		}
		break;
	}
	case RampGen::Single:
	{
		...
		{
			output[0] = (HiLevel + LoLevel) / 2;
		}
		else if (t >= InitialDelay + BurstDelay && t < InitialDelay + BurstDelay + BurstLength)
		{
			if (pt - BurstDelay < tInflection)
			{
				output[0] = Polarity ? 
					HiLevel + (LoLevel - HiLevel)*(pt - BurstDelay) / tInflection :
					LoLevel + (HiLevel - LoLevel)*(pt - BurstDelay) / tInflection;
			}
			else
			{
				output[0] = Polarity ?
					LoLevel + (HiLevel - LoLevel)*(pt - BurstDelay - tInflection) / (1 / Frequency - tInflection) :
					HiLevel + (LoLevel - HiLevel)*(pt - BurstDelay - tInflection) / (1 / Frequency - tInflection);
			}
		}
		else
		{
