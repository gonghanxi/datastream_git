...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE ( BPF_ChebyshevI )
{	
	SET_MODEL_DESCRIPTION("Bandpass Chebyshev I Filter");
	...
	{
		SystemVueModelBuilder::DFParam param = ADD_MODEL_PARAMETER(PassRipple);
...
BPF_ChebyshevI::BPF_ChebyshevI()
{
	
}

ERESULT BPF_ChebyshevI::PropagateCharacterizationFrequency()
{
	bool bStatus = true;
...
bool BPF_ChebyshevI::Setup()
{
	...
	// 滤波器初始化
	double	CutoffFreq = PassBandwidth / 2;

	if (SampleRate > PassBandwidth)
	{
		shelfFilterReal.setup(FilterOrder, SampleRate, CutoffFreq, PassRipple);
		shelfFilterImag.setup(FilterOrder, SampleRate, CutoffFreq, PassRipple);
	}

	else if (SampleRate <= PassBandwidth)
	{
		// 欠采样时全通输出
		if (UnderSampledModel == ModelAsAllpass)
		{
            char str[256];
            sprintf(str, "Model as all-pass in the under-sampling situation. SampleRate (%f) should be larger than PassBandwidth (%f).", SampleRate, PassBandwidth);
		...
		{
            char str[256];
            sprintf(str, "SampleRate (%f) should be larger than PassBandwidth (%f).", SampleRate, PassBandwidth);
			POST_ERROR(str);
...
// 计算 exp(j * 2 * PI * f_c * t)
std::complex<double> BPF_ChebyshevI::complexExponential(double f_c, double t) {
	// 计算角频率：ω = 2 * π * f_c
...
// 分贝转倍数
double BPF_ChebyshevI::dBToPowerRatio(double dB) {
