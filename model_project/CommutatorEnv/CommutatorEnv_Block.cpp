#include "CommutatorEnv_Block.h"
#include <vector>

CommutatorEnv_Block::CommutatorEnv_Block(const std::string& name)
    : Block(name)
{
}

void CommutatorEnv_Block::SetDefaultParamters()
{
    m_blockSize = 1;
    m_iBlockSize = 1U;
}

void CommutatorEnv_Block::SetParameters()
{
    if (!m_commutatorEnv) {
        return;
    }

    m_commutatorEnv->BlockSize = m_blockSize;
    m_commutatorEnv->m_iBlockSize = m_iBlockSize;
}

bool CommutatorEnv_Block::ModelSetup()
{
...
bool CommutatorEnv_Block::Setup()
{
    Block::Setup();
...
bool CommutatorEnv_Block::Run()
{
    if(IsVariableStepMode()) return TimeDrivenRun();
    return DataStreamRun();
}

bool CommutatorEnv_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_commutatorEnv = std::make_unique<CommutatorEnv>();

    AddInputPort("input", m_commutatorEnv->input, 1, Block::DataType::ENVELOPE_BUS);
    AddOutputPort("output", m_commutatorEnv->output, 1, Block::DataType::ENVELOPE_SIGNAL);
...
bool CommutatorEnv_Block::DataStreamRun()
{
...
bool CommutatorEnv_Block::TimeDrivenRun()
{
    ...
    std::vector<EnvelopeSignal> outputData(m_iBlockSize * bridge_readers.size());  // 初始化为0
