#ifndef SINKENV_BLOCK_H
#define SINKENV_BLOCK_H
...
{
public:
    SinkEnv_Block(const std::string& name);
    ~SinkEnv_Block();
    ...
    void SetParameters(int SampleStart = 0, int SampleStop = 1,
                       ...
                       SinkEnv::SelectedStartStopOption StartStopOption = SinkEnv::Auto,
                       char* Filename = nullptr);
private:
    SinkEnv::SelectedStartStopOption ConvertStringToSelected(const std::string& value);
    ...
    SinkEnv::SelectedStartStopOption m_StartStopOption;
    int m_SampleStart;
    int m_SampleStop;
    double m_TimeStart;
    double m_TimeStop;
    ...
    // SINK输出截断控制
    unsigned long long m_sinkTargetSamples = ULLONG_MAX; // 最大输出采样点数，默认无限制
