...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE(Demapper)
{
	SET_MODEL_DESCRIPTION("Complex Symbol Demapper/Slicer");
	SET_MODEL_SYMBOL("SYM_Demapper");
	SET_MODEL_CATEGORY("Communications");

	ADD_MODEL_INPUT(m_input);
	ADD_MODEL_OUTPUT(m_bits);
	ADD_MODEL_OUTPUT(m_node);

	{
		DFParam e = ADD_MODEL_ENUM_PARAM(ModType, Demapper::ModTypeEnum);
		e.SetUnit(Units::NONE);
		e.AddEnumeration("BPSK", Demapper::BPSK);
		e.AddEnumeration("QPSK", Demapper::QPSK);
		e.AddEnumeration("8-PSK", Demapper::PSK8);
		e.AddEnumeration("16-PSK", Demapper::PSK16);
		e.AddEnumeration("16-QAM", Demapper::QAM16);
		e.AddEnumeration("32-QAM", Demapper::QAM32);
		e.AddEnumeration("64-QAM", Demapper::QAM64);
		e.AddEnumeration("128-QAM", Demapper::QAM128);
		e.AddEnumeration("256-QAM", Demapper::QAM256);
		e.AddEnumeration("User Defined", Demapper::User_Defined);
		e.AddEnumeration("512-QAM", Demapper::QAM512);
		e.AddEnumeration("1024-QAM", Demapper::QAM1024);
		e.AddEnumeration("2048-QAM", Demapper::QAM2048);
		e.AddEnumeration("4096-QAM", Demapper::QAM4096);
		e.AddEnumeration("16-APSK", Demapper::APSK16);
		e.AddEnumeration("32-APSK", Demapper::APSK32);
		e.AddEnumeration("Star 16-QAM", Demapper::Star16QAM);
		e.AddEnumeration("Star 32-QAM", Demapper::Star32QAM);
		e.AddEnumeration("Custom APSK", Demapper::CustomAPSK);
	...
	{
		DFParam e = ADD_MODEL_ENUM_PARAM(DefaultState, Demapper::DefaultStateEnum);
		e.SetUnit(Units::NONE);
		e.AddEnumeration("FALSE", Demapper::FALSE_);
		e.AddEnumeration("TRUE", Demapper::TRUE_);
	...
	{
		DFParam e = ADD_MODEL_ENUM_PARAM(BitOrder, Demapper::BitOrderEnum);
		e.SetUnit(Units::NONE);
		e.AddEnumeration("LSB first", Demapper::LSB_first);
		e.AddEnumeration("MSB first", Demapper::MSB_first);
...
bool Demapper::Initialize()
{
	return true;
}

bool Demapper::Finalize()
{
	return true;
}

unsigned int Demapper::GrayEncode(unsigned int v)
{
	return v ^ (v >> 1);
}

unsigned int Demapper::GrayDecode(unsigned int g)
{
	unsigned int b = 0u;
	while (g != 0u)
	{
...
bool Demapper::IsPowerOfTwo(int n)
{
	return (n > 0) && ((n & (n - 1)) == 0);
}

int Demapper::ILog2Pow2(int n)
{
	int k = 0;
	while ((1 << k) < n)
		++k;
...
double Demapper::DegToRad(double deg)
{
	return deg * SV_PI / 180.0;
}

std::complex<double> Demapper::Polar(double r, double phase)
{
	return std::complex<double>(r * std::cos(phase), r * std::sin(phase));
}

std::complex<double> Demapper::RotateByQuadrant(int q2, double I, double Q)
{
	switch (q2 & 0x3)
	{
	case 0b00: return std::complex<double>(I, Q);
...
std::complex<double> Demapper::RotateByQuadrantGray(int q2, double I, double Q)
{
	switch (q2 & 0x3)
	{
	case 0b00: return std::complex<double>(I, Q);
...
void Demapper::NormalizeTable(std::vector<std::complex<double> >& table)
{
	if (table.empty())
		return;
...
bool Demapper::ShouldNormalize() const
{
...
bool Demapper::Setup()
{
	...
	TrySetRate(m_input, 1, 0);
	TrySetRate(m_bits, m_symbolLength, 0);
	TrySetRate(m_node, 1, 0);
...
bool Demapper::Run()
{
	...
	const std::complex<double> x = m_input[0U];
	...
	m_node[0U] = m_table[static_cast<size_t>(idx)];

	int state = idx;
	if (ModType == CustomAPSK && DefaultState == FALSE_)
	{
		if (static_cast<int>(m_indexToState.size()) != m_M)
		{
            LOG_ERROR("Custom APSK index-to-state map is invalid.");
			return false;
		}
		state = m_indexToState[static_cast<size_t>(idx)];
		if (state < 0 || state >= m_M)
		{
            LOG_ERROR("Custom APSK state out of range.");
			return false;
		}
	}

	WriteBitsFromState(state);
	return true;
}

int Demapper::FindNearestIndex(const std::complex<double>& x) const
{
	int bestIdx = -1;
	...
	for (int i = 0; i < m_M; ++i)
	{
		const double d = std::norm(x - m_table[static_cast<size_t>(i)]);
		if (d < bestDist)
		{
			bestDist = d;
			bestIdx = i;
...
void Demapper::WriteBitsFromState(int state)
{
	if (BitOrder == LSB_first)
	{
		for (int i = 0; i < m_symbolLength; ++i)
			m_bits[static_cast<unsigned>(i)] = ((state >> i) & 0x1) != 0;
	}
	else
	{
		for (int i = 0; i < m_symbolLength; ++i)
			m_bits[static_cast<unsigned>(i)] = ((state >> (m_symbolLength - 1 - i)) & 0x1) != 0;
	}
}

bool Demapper::RebuildConstellation()
{
	...
	if (ModType == CustomAPSK && DefaultState == FALSE_)
		return BuildIndexToStateMap(m_M);

	m_indexToState.clear();
	return true;
}

void Demapper::BuildBPSK(std::vector<std::complex<double> >& table) const
{
...
void Demapper::BuildPSK(int M, double phase0_rad, std::vector<std::complex<double> >& table) const
{
...
void Demapper::BuildQAM(int M, std::vector<std::complex<double> >& table) const
{
	table.assign(static_cast<size_t>(M), std::complex<double>(0.0, 0.0));
...
void Demapper::BuildUserDefined(std::vector<std::complex<double> >& table) const
{
	const int N = MatrixNumElements(MappingTable);
...
void Demapper::BuildAPSK16(std::vector<std::complex<double> >& table) const
{
...
void Demapper::BuildAPSK32(std::vector<std::complex<double> >& table) const
{
...
void Demapper::BuildStar16(std::vector<std::complex<double> >& table) const
{
	table.assign(16, std::complex<double>(0.0, 0.0));
...
void Demapper::BuildStar32(std::vector<std::complex<double> >& table) const
{
	table.assign(32, std::complex<double>(0.0, 0.0));
...
bool Demapper::BuildCustomAPSK(std::vector<std::complex<double> >& table)
{
	const int nr = MatrixNumElements(RingStates);
	const int nm = MatrixNumElements(RingMagnitudes);
	const int np = MatrixNumElements(RinginitialPhases);
...
bool Demapper::BuildIndexToStateMap(int M)
{
	...
	m_indexToState.assign(static_cast<size_t>(M), -1);
	std::set<int> used;
	...
	for (int i = 0; i < M; ++i)
	{
		...
		m_indexToState[static_cast<size_t>(i)] = s;
	}
