...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE ( LPF_ChebyshevI )
{	
	SET_MODEL_DESCRIPTION("Lowpass Chebyshev I Filter");
...
LPF_ChebyshevI::LPF_ChebyshevI()
{

}

ERESULT LPF_ChebyshevI::PropagateCharacterizationFrequency()
{
	bool bStatus = true;
...
bool LPF_ChebyshevI::Setup()
{
	...
	{
		// 滤波器初始化
		shelfFilterReal.setup(FilterOrder, SampleRate, PassFreq, PassRipple);
		shelfFilterImag.setup(FilterOrder, SampleRate, PassFreq, PassRipple);
	}
	...
	else if (SampleRate <= 2 * PassFreq)
	{
		...
		{
            char str[256];
            sprintf(str, "Model as all-pass in the under-sampling situation. SampleRate (%f) should be larger than 2 * PassFreq (%f).", SampleRate, 2 * PassFreq);
			POST_WARNING(str);
...
// 计算 exp(j * 2 * PI * f_c * t)
std::complex<double> LPF_ChebyshevI::complexExponential(double f_c, double t) {
	// 计算角频率：ω = 2 * π * f_c
...
// 分贝转倍数
double LPF_ChebyshevI::dBToPowerRatio(double dB) {
