...
ChopCx_Block::ChopCx_Block(const std::string& name)
    : Block(name)
{
}

void ChopCx_Block::SetDefaultParamters()
{
    m_nRead = 128;
    m_nWrite = 64;
    m_offset = 0;
...
void ChopCx_Block::SetParameters()
{
    if (!m_chop) {
        return;
    }
...
bool ChopCx_Block::Run()
{
    if(IsVariableStepMode()) return TimeDrivenRun();
    return DataStreamRun();
}

bool ChopCx_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_chop = std::make_unique<SystemVueModelBuilder::ChopCx>();

    AddInputPort("input", m_chop->input, 1, Block::DataType::CIRCULAR_BUFFER_DCOMPLEX);
    AddOutputPort("output", m_chop->output, 1, Block::DataType::CIRCULAR_BUFFER_DCOMPLEX);
    ...
    m_history.clear();
    if ((m_usePastInputs == QUERY_YES) && (m_offset > 0)) {
        m_history.resize(m_iReadBufSize, std::complex<double>(0.0, 0.0));
    }

    return true;
}

QueryEnum ChopCx_Block::ConvertStringToQueryEnum(const std::string& value)
{
...
bool ChopCx_Block::InitializeRanges()
{
    if ((m_usePastInputs == QUERY_YES) && (m_offset > 0)) {
        m_iReadFrom = 0;
        m_iWriteTo = 0;
...
bool ChopCx_Block::DataStreamRun()
{
    ...
    if ((m_usePastInputs == QUERY_YES) && (m_offset > 0)) {
        for (const std::complex<double>& v : inputData) {
        ...
        if (m_history.size() < m_iReadBufSize) {
            ...
            m_history.insert(m_history.begin(), pad, std::complex<double>(0.0, 0.0));
        }
    } else {
    ...
    std::vector<std::complex<double>> outputData(m_nWrite, std::complex<double>(0.0, 0.0));
...
bool ChopCx_Block::TimeDrivenRun()
{
    std::string inputPort = GetInputPortName(0);
    std::string outputPort = GetOutputPortName(0);
    ...
    if(m_inputBuffer.size() >= static_cast<size_t>(m_nRead)) {
        if ((m_usePastInputs == QUERY_YES) && (m_offset > 0)) {
            for (std::complex<double> v : m_inputBuffer) {
                m_history.push_back(v);
            }
            while (m_history.size() > m_iReadBufSize) {
                m_history.pop_front();
        ...
        std::vector<std::complex<double>> outputData(m_nWrite);
