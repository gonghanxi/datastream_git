...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE ( DownSampleEnv )
{	
	SET_MODEL_DESCRIPTION("Down Sampler for Envelope Signal");
	SET_MODEL_SYMBOL("SYM_DownSampleEnv");
...
DownSampleEnv::DownSampleEnv()
{
	
}

ERESULT DownSampleEnv::PropagateCharacterizationFrequency()
{
	output.SetCharacterizationFrequency(input.GetCharacterizationFrequency());
	return true;
}

bool DownSampleEnv::Setup()
{
	bool bStatus = true;
...
//		Here we do the math
//-----------------------------------------------------------------------------------
bool DownSampleEnv::Run()
{
	output[0] = input[Factor];
	return true;
}
