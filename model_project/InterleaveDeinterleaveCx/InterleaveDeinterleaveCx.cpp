...
{
	DEFINE_MODEL_INTERFACE(InterleaveDeinterleaveCx)
	{
...
namespace SystemVueModelBuilder
{
	InterleaveDeinterleaveCx::InterleaveDeinterleaveCx()
		: Rows(8), Columns(8)
	{
	}

	bool InterleaveDeinterleaveCx::Setup()
	{
		if (Columns > 0 && Rows > 0)
		{
	...
	bool InterleaveDeinterleaveCx::Initialize()
	{
		bool bStatus = true;
		if (Rows < 1)
		{
	...
	bool InterleaveDeinterleaveCx::Run()
	{
		// 与 SystemVue 模板实现一致：
