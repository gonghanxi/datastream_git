...
Chop_Block::Chop_Block(const std::string& name)
    : Block(name)
{
}

void Chop_Block::SetDefaultParamters()
{
    m_nRead = 128;
    m_nWrite = 64;
    m_offset = 0;
...
bool Chop_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_chop = std::make_unique<SystemVueModelBuilder::Chop>();

    AddInputPort("input", m_chop->input, 1, Block::DataType::CIRCULAR_BUFFER_DOUBLE);
    AddOutputPort("output", m_chop->output, 1, Block::DataType::CIRCULAR_BUFFER_DOUBLE);
...
QueryEnum Chop_Block::ConvertStringToQueryEnum(const std::string& value)
{
...
bool Chop_Block::InitializeRanges()
{
    if ((m_usePastInputs == QUERY_YES) && (m_offset > 0)) {
        m_iReadFrom = 0;
        m_iWriteTo = 0;
...
bool Chop_Block::DataStreamRun()
{
    ...
    std::vector<double> outputData(m_nWrite, 0.0);
...
bool Chop_Block::TimeDrivenRun()
{
    ...
    if(m_inputBuffer.size() >= static_cast<size_t>(m_nRead)) {
        ...
        std::vector<double> outputData(m_nWrite);
