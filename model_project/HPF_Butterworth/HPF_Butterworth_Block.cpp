#include "HPF_Butterworth_Block.h"
#include <algorithm>
#include <cctype>
#include <cmath>
#include <complex>
...
HPF_Butterworth_Block::HPF_Butterworth_Block(const std::string& name)
    : Block(name)
{
}

void HPF_Butterworth_Block::SetDefaultParamters()
{
    m_loss = 0.0;
    m_passFreq = 150e3;
    m_passAtten = 3.0;
    m_stopFreq = 100e3;
    m_stopAtten = 50.0;
    m_orderType = HPF_Butterworth::UserDefined;
    m_order = 5;
    m_sampleRate = getSimu().samplingRate;
}

void HPF_Butterworth_Block::SetParameters()
{
    if (!m_hpf) {
        return;
    }
...
bool HPF_Butterworth_Block::Setup()
{
    Block::Setup();
    return true;
}

bool HPF_Butterworth_Block::Run()
{
...
bool HPF_Butterworth_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_hpf = std::make_unique<HPF_Butterworth>();
...
void HPF_Butterworth_Block::UpdateCharacterizationFrequency()
{
...
HPF_Butterworth::SelectedOrderType HPF_Butterworth_Block::ConvertStringToOrderType(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "userdefined") {
        return HPF_Butterworth::UserDefined;
    }
    if (lower == "auto" || lower == "0") {
        return HPF_Butterworth::Auto;
    }
    return HPF_Butterworth::UserDefined;
}
