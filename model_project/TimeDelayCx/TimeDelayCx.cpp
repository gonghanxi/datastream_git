...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE(TimeDelayCx)
{
...
bool TimeDelayCx::Setup()
{
	input.SetRate(1U);
	output.SetRate(1U);

	if (T < 0.0) {
		POST_ERROR("TimeDelayCx: T must be >= 0.");
		return false;
	}
	if (N < 0) {
		POST_ERROR("TimeDelayCx: N must be >= 0.");
		return false;
	}
...
ERESULT TimeDelayCx::CalculateLatency()
{
	...
	if (Unit == Unit_Time) {
		...
		if (dtIn <= 0.0) {
			POST_ERROR("TimeDelayCx: input signal must be timed (sample rate > 0).");
			return -1;
		}
