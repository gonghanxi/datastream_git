...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE ( LPF_Butterworth )
{	
	SET_MODEL_DESCRIPTION("Lowpass Butterworth Filter");
...
LPF_Butterworth::LPF_Butterworth()
{

}

ERESULT LPF_Butterworth::PropagateCharacterizationFrequency()
{
	bool bStatus = true;
...
// 计算 exp(j * 2 * PI * f_c * t)
std::complex<double> LPF_Butterworth::complexExponential(double f_c, double t) {
	// 计算角频率：ω = 2 * π * f_c
...
// 分贝转倍数
double LPF_Butterworth::dBToPowerRatio(double dB) {
