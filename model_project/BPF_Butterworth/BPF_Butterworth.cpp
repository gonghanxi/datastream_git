...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE ( BPF_Butterworth )
{	
	SET_MODEL_DESCRIPTION("Bandpass Butterworth Filter");
...
BPF_Butterworth::BPF_Butterworth()
{

}

ERESULT BPF_Butterworth::PropagateCharacterizationFrequency()
{
	bool bStatus = true;
...
bool BPF_Butterworth::Setup()
{
	...
	{
		shelfFilterReal.setup(FilterOrder, SampleRate, CutoffFreq);
		shelfFilterImag.setup(FilterOrder, SampleRate, CutoffFreq);
	}
...
// 计算 exp(j * 2 * PI * f_c * t)
std::complex<double> BPF_Butterworth::complexExponential(double f_c, double t) {
	// 计算角频率：ω = 2 * π * f_c
...
// 分贝转倍数
double BPF_Butterworth::dBToPowerRatio(double dB) {
