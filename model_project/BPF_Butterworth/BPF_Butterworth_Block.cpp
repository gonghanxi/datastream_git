#include "BPF_Butterworth_Block.h"
#include <algorithm>
#include <cctype>
#include <complex>
#include <vector>
...
BPF_Butterworth_Block::BPF_Butterworth_Block(const std::string& name)
    : Block(name)
{
}

void BPF_Butterworth_Block::SetDefaultParamters()
{
    m_loss = 0.0;
    m_fCenter = 150e3;
    ...
    m_stopAtten = 50.0;
    m_orderType = BPF_Butterworth::UserDefined;
    m_order = 5;
    m_transform = BPF_Butterworth::Bilinear;
    m_underSampledModel = BPF_Butterworth::ModelAsAllpass;
    m_sampleRate = getSimu().samplingRate;
}

void BPF_Butterworth_Block::SetParameters()
{
    if (!m_bpf) {
        return;
    }
...
bool BPF_Butterworth_Block::Setup()
{
    Block::Setup();
    return true;
}

bool BPF_Butterworth_Block::Run()
{
...
bool BPF_Butterworth_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_bpf = std::make_unique<BPF_Butterworth>();
...
//    double CutoffFreq = m_passBandwidth / 2;
//    if(m_sampleRate < CutoffFreq) {
//        LOG_ERROR("BPF_ButterWorth_", GetName(), ": Simulator SampleRate(", m_sampleRate ,") must be greater than PassBandWidth / 2(", CutoffFreq ,")");
//        return false;
//    }
    SetParameters();
...
void BPF_Butterworth_Block::UpdateCharacterizationFrequency()
{
...
BPF_Butterworth::SelectedOrderType BPF_Butterworth_Block::ConvertStringToOrderType(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "userdefined") {
        return BPF_Butterworth::UserDefined;
    }
    if (lower == "auto" || lower == "0") {
        return BPF_Butterworth::Auto;
    }
    return BPF_Butterworth::UserDefined;
}

BPF_Butterworth::SelectedTransform BPF_Butterworth_Block::ConvertStringToTransform(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "bilinear") {
        return BPF_Butterworth::Bilinear;
    }
    if (lower == "impulse invariance" || lower == "impulseinvariance" || lower == "1") {
        return BPF_Butterworth::ImpulseInvariance;
    }
    return BPF_Butterworth::Bilinear;
}

BPF_Butterworth::SelectedUnderSampledModel BPF_Butterworth_Block::ConvertStringToUnderSampledModel(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "modelasallpass" || lower == "0") {
        return BPF_Butterworth::ModelAsAllpass;
    }
    if (lower == "error out" || lower == "errorout" || lower == "1") {
        return BPF_Butterworth::ErrorOut;
    }
    return BPF_Butterworth::ModelAsAllpass;
}
