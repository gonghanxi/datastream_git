#include "Average_Block.h"

Average_Block::Average_Block(const std::string &name)
    :Block(name)
{

}

bool Average_Block::Setup()
{
    Block::Setup();
...
bool Average_Block::Run()
{
    if(IsVariableStepMode()) return TimeDrivenRun();
    return DataStreamRun();
}

bool Average_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);
    m_Average = std::make_unique<Average>();
    ...
    AddInputPort("input", m_Average->input, static_cast<size_t>(m_NumInputsToAverage * m_BlockSize), DataType::CIRCULAR_BUFFER_DOUBLE);
    AddOutputPort("output", m_Average->output, static_cast<size_t>(m_BlockSize), DataType::CIRCULAR_BUFFER_DOUBLE);
    return true;
}

void Average_Block::SetParameters()
{
    if(!m_Average) return;
...
void Average_Block::SetDefaultParameters()
{
    m_NumInputsToAverage = 8;
    m_BlockSize = 1;
}

bool Average_Block::DataStreamRun()
{
    std::string inputPort = GetInputPortName(0);
    std::string outputPort = GetOutputPortName(0);

    std::vector<double> inputData = ReadInputData<double>(inputPort);
    std::vector<double> outputData(m_BlockSize);
...
bool Average_Block::TimeDrivenRun()
{
    std::string inputPort = GetInputPortName(0);
    std::string outputPort = GetOutputPortName(0);
