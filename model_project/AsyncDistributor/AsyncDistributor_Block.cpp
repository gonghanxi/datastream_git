#include "AsyncDistributor_Block.h"

AsyncDistributor_Block::AsyncDistributor_Block(const std::string &name)
    :Block(name)
{

}

bool AsyncDistributor_Block::Setup()
{
    Block::Setup();
...
bool AsyncDistributor_Block::Run()
{
    if(IsVariableStepMode()) return TimeDrivenRun();
    return DataStreamRun();
}

bool AsyncDistributor_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_AsyncDistributor = std::make_unique<AsyncDistributor>();
    ...
    AddInputPort("input", m_AsyncDistributor->input, 1, DataType::CIRCULAR_BUFFER_DOUBLE);
    AddOutputPort("output", m_AsyncDistributor->output, 1, DataType::DOUBLE_BUS);
    return true;
}

void AsyncDistributor_Block::SetParameters()
{
    if(!m_AsyncDistributor) return;
    m_AsyncDistributor->BlockSizes = m_BlockSizes;
}

void AsyncDistributor_Block::SetDefaultParameters()
{
    m_BlockSizes.Resize(1,1);
    m_BlockSizes(0,0) = 1;
}

bool AsyncDistributor_Block::ModelsSetup()
{
    ...
    for (size_t i = 0; i < numOutputs; ++i)
    {
        ...
        qDebug() << "AsyncDistributor_Block::ModelsSetup - Bi: " << Bi;
        numInputData += static_cast<size_t>(Bi);
        auto& bus = outputBuffer->GetBusConnections()[i];
        bus.bridgeWriter->SetWriteSize(static_cast<size_t>(Bi));
    }
    ...
    qDebug() << "AsyncDistributor_Block::ModelsSetup - numInputData: " << numInputData;
...
bool AsyncDistributor_Block::DataStreamRun()
{
...
bool AsyncDistributor_Block::TimeDrivenRun()
{
    std::string inputPort = GetInputPortName(0);
    std::string outputPort = GetOutputPortName(0);
    ...
    for (size_t i = 0; i < m_channelQueues.size(); ++i) {
        if (!m_channelQueues[i].empty()) {
            double sample = m_channelQueues[i].front();
            m_channelQueues[i].pop();
            // 写入单个样本到对应通道
            outputBuffer->WriteDataToChannel(static_cast<int>(i), std::vector<double>{sample});
            anyOutput = true;
