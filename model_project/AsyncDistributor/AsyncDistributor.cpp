...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE(AsyncDistributor)
{
...
bool AsyncDistributor::Setup()
{
	...
	if (numBlockVals == 0)
	{
		POST_ERROR("AsyncDistributor: BlockSizes must contain at least one element.");
		return false;
	}
	...
	if (numBlockVals == 1)
	{
		...
		if (B <= 0)
		{
			POST_ERROR("AsyncDistributor: elements of BlockSizes array must all be > 0.");
			return false;
		}
	...
	for (size_t i = 0; i < numOutputs; ++i)
	{
		...
		if (Bi <= 0)
		{
			POST_ERROR("AsyncDistributor: expanded BlockSizes element is not > 0.");
			return false;
		}
	...
	if (numInputData == 0)
	{
		POST_ERROR("AsyncDistributor: total number of input samples per run must be > 0.");
		return false;
	}
