...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE(COMM_AntennaPolarizationRx)
{
	SET_MODEL_DESCRIPTION("Communication antenna radiation pattern, gain and polarization for RX");
...

	{
		...
		p.SetDescription("Azimuth angle of receive beam direction related to the antenna reference coordinate (radian)");
		p.SetOptional(true);
	}

	{
		SystemVueModelBuilder::DFPort p = ADD_MODEL_INPUT(BeamElevation);
		p.SetDescription("Elevation angle of receive beam direction related to the antenna reference coordinate (radian)");
		p.SetOptional(true);
	}

	{
		SystemVueModelBuilder::DFPort p = ADD_MODEL_INPUT(input_V);
		p.SetDescription("Vertical polarization input envelope signals in the communication Rx chain");
		p.SetOptional(true);
	}

	{
		SystemVueModelBuilder::DFPort p = ADD_MODEL_INPUT(input_H);
		p.SetDescription("Horizontal polarization input envelope signals in the communication Rx chain");
		p.SetOptional(true);
	}
	...
	// =====================================================================
	{
		...
		p.SetDescription("Combined vertical polarization output envelope signal of the receive antenna");
		p.SetOptional(true);
	}

	{
		SystemVueModelBuilder::DFPort p = ADD_MODEL_OUTPUT(output_H);
		p.SetDescription("Combined horizontal polarization output envelope signal of the receive antenna");
		p.SetOptional(true);
	}
	...
	// =====================================================================
	{
		...
		p.AddEnumeration("Parametric Pattern", COMM_AntennaPolarizationRx::ParametricPattern);
		p.AddEnumeration("Import Actual Pattern", COMM_AntennaPolarizationRx::ImportActualPattern);
...

	{
		...
		p.AddEnumeration("Isotropic", COMM_AntennaPolarizationRx::IsotropicPattern);
		p.AddEnumeration("Cosine Power", COMM_AntennaPolarizationRx::CosinePowerPattern);
		p.AddEnumeration("Gaussian", COMM_AntennaPolarizationRx::GaussianPattern);
		p.AddEnumeration("3GPP", COMM_AntennaPolarizationRx::ThreeGPPPattern);
	...
	// =====================================================================
	{
		...
		p.AddEnumeration("Pattern Components", COMM_AntennaPolarizationRx::PatternComponents);
		p.AddEnumeration("Horizontal", COMM_AntennaPolarizationRx::HorizontalPolarization);
		p.AddEnumeration("Vertical", COMM_AntennaPolarizationRx::VerticalPolarization);
		p.AddEnumeration("Linear Slant", COMM_AntennaPolarizationRx::LinearSlantPolarization);
		p.AddEnumeration("RHCP", COMM_AntennaPolarizationRx::RHCPPolarization);
		p.AddEnumeration("LHCP", COMM_AntennaPolarizationRx::LHCPPolarization);
		p.AddEnumeration("User Defined Jones", COMM_AntennaPolarizationRx::UserDefinedJones);
	...
	// =====================================================================
	{
		...
		p.AddEnumeration("EMPro", COMM_AntennaPolarizationRx::EMPro);
		p.AddEnumeration("HFSS", COMM_AntennaPolarizationRx::HFSS);
		p.AddEnumeration("CST", COMM_AntennaPolarizationRx::CST);
...

	{
		...
		p.AddEnumeration("UserDefine2D", COMM_AntennaPolarizationRx::UserDefine2D);
		p.AddEnumeration("UserDefine3D", COMM_AntennaPolarizationRx::UserDefine3D);
...

	{
		...
		p.AddEnumeration("Use File Gain", COMM_AntennaPolarizationRx::UseFileGain);
		p.AddEnumeration("Normalize File To Peak Gain", COMM_AntennaPolarizationRx::NormalizeFileToPeakGain);
...

	{
		...
		p.SetDescription("Communication Rx antenna pattern filename");
	...
	// =====================================================================
	{
		...
		p.AddEnumeration("Fixed Beam", COMM_AntennaPolarizationRx::FixedBeam);
		p.AddEnumeration("Beam Sweep", COMM_AntennaPolarizationRx::BeamSweep);
		p.SetDescription("Communication receive beam control mode");
		p.SetDefaultValue("0");
	}

	{
		...
		p.AddEnumeration("Circular", COMM_AntennaPolarizationRx::CircularScan);
		p.AddEnumeration("Bidirectional Sector", COMM_AntennaPolarizationRx::BidirectionalSector);
		p.AddEnumeration("Unidirectional Sector", COMM_AntennaPolarizationRx::UnidirectionalSector);
		p.AddEnumeration("Bidirectional Raster", COMM_AntennaPolarizationRx::BidirectionalRaster);
		p.AddEnumeration("Unidirectional Raster", COMM_AntennaPolarizationRx::UnidirectionalRaster);
		p.SetDescription("Communication receive beam sweep pattern");
		p.SetDefaultValue("0");
...
COMM_AntennaPolarizationRx::COMM_AntennaPolarizationRx() :
...
bool COMM_AntennaPolarizationRx::Setup()
{
	...
	if (PatternDataMode == ImportActualPattern)
	{
		if (!loadPatternFile())
		{
			POST_ERROR("COMM_AntennaPolarizationRx: failed to load the selected antenna pattern file.");
			return false;
		}
	}
	else
...
ERESULT COMM_AntennaPolarizationRx::PropagateCharacterizationFrequency()
{
	double fc = 0.0;
...
bool COMM_AntennaPolarizationRx::Run()
{
	const int directionCount = determineDirectionCount();

	std::complex<double> summedV(0.0, 0.0);
	std::complex<double> summedH(0.0, 0.0);
	...
	for (size_t i = 0; i < input_V.GetSize(); ++i)
	{
		if (input_V[i].IsConnected())
		{
			timeNow = input_V[i].GetTime(0, GetCount());
			timeFound = true;
			break;
		}
	}
	...
	if (!timeFound)
	{
		for (size_t i = 0; i < input_H.GetSize(); ++i)
		{
			if (input_H[i].IsConnected())
			{
				timeNow = input_H[i].GetTime(0, GetCount());
				break;
			}
		}
	}
	...
	for (int ch = 0; ch < directionCount; ++ch)
	{
		...
		std::complex<double> responseV(0.0, 0.0);
		std::complex<double> responseH(0.0, 0.0);
		...
		if (PatternDataMode == ImportActualPattern)
		{
			...
			if (ImportedGainMode == NormalizeFileToPeakGain)
			{
				...
				if (patternPeakAmplitude_ > 0.0)
				{
					...
					responseV *= normalizationScale;
					responseH *= normalizationScale;
				}
			}
			...
			if (PolarizationType != PatternComponents)
			{
				const std::complex<double> scalarGain =
					combinePatternComponentsToScalar(responseV, responseH);
		...
		std::complex<double> pathOutputV(0.0, 0.0);
		std::complex<double> pathOutputH(0.0, 0.0);

		applyReceivePolarization(xV,
			xH,
			responseV,
			responseH,
			pathOutputV,
...
bool COMM_AntennaPolarizationRx::hasBeamAzimuthPort()
{
	return BeamAzimuth.IsConnected();
}

bool COMM_AntennaPolarizationRx::hasBeamElevationPort()
{
	return BeamElevation.IsConnected();
}

bool COMM_AntennaPolarizationRx::hasDirectionAzimuthPort(int ch)
{
	if (ch < 0)
	{
		return false;
...
bool COMM_AntennaPolarizationRx::hasDirectionElevationPort(int ch)
{
	if (ch < 0)
	{
		return false;
...
bool COMM_AntennaPolarizationRx::hasInputVPort(int ch)
{
	if (ch < 0)
	{
		return false;
...
bool COMM_AntennaPolarizationRx::hasInputHPort(int ch)
{
	if (ch < 0)
	{
		return false;
...
int COMM_AntennaPolarizationRx::determineDirectionCount()
{
	int n = 0;
...
double COMM_AntennaPolarizationRx::getArrayValue(const double* data,
	int size,
	int index,
...
double COMM_AntennaPolarizationRx::getScaleValue(int index) const
{
...
void COMM_AntennaPolarizationRx::getBeamAngle(double timeNow,
	double& beamAzRad,
	double& beamElRad)
{
...
double COMM_AntennaPolarizationRx::getCircularScanAzimuth(double timeNow) const
{
...
double COMM_AntennaPolarizationRx::getSectorScanAzimuth(double timeNow,
...
void COMM_AntennaPolarizationRx::getRasterScanAngle(double timeNow,
	bool bidirectional,
	double& azDeg,
	double& elDeg) const
{
...
bool COMM_AntennaPolarizationRx::validateConfiguration() const
{
...
double COMM_AntennaPolarizationRx::calculateCosineExponent(
...
double COMM_AntennaPolarizationRx::calculateParametricAttenuationDb(
	double relAzRad,
...
std::complex<double> COMM_AntennaPolarizationRx::calculateParametricScalarGain(
	double relAzRad,
	double relElRad) const
{
...
void COMM_AntennaPolarizationRx::buildPolarizationJones(
...
void COMM_AntennaPolarizationRx::applyConfiguredPolarization(
	const std::complex<double>& scalarGain,
...
void COMM_AntennaPolarizationRx::applyReceivePolarization(
	const std::complex<double>& inputV,
...
{
	outputV = std::complex<double>(0.0, 0.0);
	outputH = std::complex<double>(0.0, 0.0);

	const double totalAmplitude =
		std::sqrt(std::norm(responseV) + std::norm(responseH));
	...
	const std::complex<double> dominant =
		(std::abs(responseV) >= std::abs(responseH))
		? responseV
		: responseH;

	const double dominantAmplitude = std::abs(dominant);

	std::complex<double> scalarGain(totalAmplitude, 0.0);

	if (dominantAmplitude > 0.0)
	{
		scalarGain = dominant * (totalAmplitude / dominantAmplitude);
	}

	if (std::abs(scalarGain) <= 0.0)
	{
		return;
	}

	std::complex<double> jonesV = responseV / scalarGain;
	std::complex<double> jonesH = responseH / scalarGain;

	normalizeJones(jonesV, jonesH);

	// Receive polarization matching uses the Hermitian inner product.
	// The scalar antenna response is conjugated so that a reciprocal
	// Tx/Rx pair with the same complex pattern produces |G|^2 gain and
	// cancels the antenna pattern phase.
	const std::complex<double> matchedVoltage =
		std::conj(jonesV) * inputV +
		std::conj(jonesH) * inputH;

	const std::complex<double> receivedVoltage =
		std::conj(scalarGain) * matchedVoltage;

	outputV = jonesV * receivedVoltage;
	outputH = jonesH * receivedVoltage;
}

std::complex<double> COMM_AntennaPolarizationRx::combinePatternComponentsToScalar(
...
void COMM_AntennaPolarizationRx::azelToPatternThetaPhi(double relAzRad,
	double relElRad,
	double& thetaDeg,
	double& phiDeg) const
{
...
void COMM_AntennaPolarizationRx::clearPattern()
{
	patternTable_.clear();
	patternLoaded_ = false;
	patternPeakAmplitude_ = 0.0;
...
bool COMM_AntennaPolarizationRx::loadPatternFile()
{
	clearPattern();
	...
	const std::string fileName = RxAntennaPatternFileName1;
...
bool COMM_AntennaPolarizationRx::parseParameterLine(const std::string& line)
{
...
bool COMM_AntennaPolarizationRx::parseNumericLine(const char* line,
...
void COMM_AntennaPolarizationRx::lookupImportedPolarizationGain(
	double thetaDeg,
	double phiDeg,
	std::complex<double>& Gtheta,
	std::complex<double>& Gphi) const
...
std::string COMM_AntennaPolarizationRx::trim(const std::string& s)
{
	size_t begin = 0;
...
std::string COMM_AntennaPolarizationRx::lowerString(const std::string& s)
{
	std::string out = s;
...
double COMM_AntennaPolarizationRx::deg2rad(double x)
{
	return x * M_PI / 180.0;
}

double COMM_AntennaPolarizationRx::rad2deg(double x)
{
	return x * 180.0 / M_PI;
}

double COMM_AntennaPolarizationRx::normalizeRad(double x)
{
	while (x > M_PI)
	{
		x -= 2.0 * M_PI;
...
double COMM_AntennaPolarizationRx::wrap360(double x)
{
...
double COMM_AntennaPolarizationRx::clamp(double x,
	double lo,
	double hi)
{
	if (x < lo)
...
double COMM_AntennaPolarizationRx::angleDiffDeg(double a,
	double b)
{
...
double COMM_AntennaPolarizationRx::linearAmplitudeFromDb(double gainDb)
{
	return std::pow(10.0, gainDb / 20.0);
}

std::complex<double> COMM_AntennaPolarizationRx::magPhaseToComplex(
	double magnitude,
	double phase,
	bool magnitudeInDb,
	bool phaseInDegrees)
...
void COMM_AntennaPolarizationRx::normalizeJones(
	std::complex<double>& jonesV,
