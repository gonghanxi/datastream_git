...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE(TimeDelayInt)
{
	SET_MODEL_DESCRIPTION("Ideal Time Delay Block, Delays the signal for a certain amount of time.");
	SET_MODEL_SYMBOL("SYM_TimeDelay");
...
bool TimeDelayInt::Setup()
{
	input.SetRate(1U);
	output.SetRate(1U);

	if (T < 0.0) {
		POST_ERROR("TimeDelayInt: T must be >= 0.");
		return false;
	}
	if (N < 0) {
		POST_ERROR("TimeDelayInt: N must be >= 0.");
		return false;
	}
...
ERESULT TimeDelayInt::CalculateLatency()
{
	...
	if (Unit == Unit_Time) {
		...
		if (dtIn <= 0.0) {
			POST_ERROR("TimeDelayInt: input signal must be timed (sample rate > 0).");
			return -1;
		}
