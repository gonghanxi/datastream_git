#include "BSF_Butterworth_Block.h"
#include <algorithm>
#include <cctype>
#include <cmath>
#include <complex>
...
BSF_Butterworth_Block::BSF_Butterworth_Block(const std::string& name)
    : Block(name)
{
}

void BSF_Butterworth_Block::SetDefaultParamters()
{
    m_loss = 0.0;
    m_fCenter = 150e3;
    ...
    m_stopAtten = 50.0;
    m_orderType = BSF_Butterworth::UserDefined;
    m_order = 5;
    m_sampleRate = getSimu().samplingRate;
}

void BSF_Butterworth_Block::SetParameters()
{
    if (!m_bsf) {
        return;
    }
...
bool BSF_Butterworth_Block::Setup()
{
    Block::Setup();
    return true;
}

bool BSF_Butterworth_Block::Run()
{
...
bool BSF_Butterworth_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_bsf = std::make_unique<BSF_Butterworth>();
...
void BSF_Butterworth_Block::UpdateCharacterizationFrequency()
{
...
BSF_Butterworth::SelectedOrderType BSF_Butterworth_Block::ConvertStringToOrderType(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "userdefined") {
        return BSF_Butterworth::UserDefined;
    }
    if (lower == "auto" || lower == "0") {
        return BSF_Butterworth::Auto;
    }
    return BSF_Butterworth::UserDefined;
}
