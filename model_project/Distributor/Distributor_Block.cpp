#include "Distributor_Block.h"

Distributor_Block::Distributor_Block(const std::string &name)
    :Block(name)
{

}

bool Distributor_Block::Setup()
{
    Block::Setup();
...
bool Distributor_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_Distributor = std::make_unique<Distributor>();
    ...
    AddInputPort("input", m_Distributor->input, 1, DataType::CIRCULAR_BUFFER_DOUBLE);
    AddOutputPort("output", m_Distributor->output, 1, DataType::DOUBLE_BUS);
    return true;
}

void Distributor_Block::SetParameters()
{
    if(!m_Distributor) return;
    m_Distributor->BlockSize = m_BlockSize;
}

void Distributor_Block::SetDefaultParameters()
{
    m_BlockSize = 1;
}

bool Distributor_Block::ModelsSetup()
{
