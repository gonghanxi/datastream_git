#ifndef SINKENV_M_BLOCK_H
#define SINKENV_M_BLOCK_H
#include "SinkEnv_M.h"
#include "Block.h"
...
class SYSTEMVUEMODELBUILDER_API SinkEnv_M_Block : public Block
{
public:
    SinkEnv_M_Block(const std::string& name);
    ~SinkEnv_M_Block();
...
private:
    ...
    SinkEnv_M::SelectedStartStopOption ConvertStringToSelected(const std::string& value);
    ...
    std::unique_ptr<SinkEnv_M> m_sink;

    SinkEnv_M::SelectedStartStopOption m_StartStopOption;
    int m_SampleStart;
    int m_SampleStop;
    double m_TimeStart;
    double m_TimeStop;
...
RegAlgo(SinkEnv_M_Block);

#endif // SINKENV_M_BLOCK_H
