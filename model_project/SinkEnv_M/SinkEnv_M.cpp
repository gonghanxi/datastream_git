...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE ( SinkEnv_M )
{	
...
bool SinkEnv_M::Setup()
{
	...
	if (bStatus)
	{
		...
		if (bStatus)
		{
			...
			m_pdBuffer = new SystemVueModelBuilder::EnvelopeMatrix[FILEWRITER_BUFFER_SIZE];
			m_iBuffer = 0;
		}
...
ERESULT SinkEnv_M::PropagateCharacterizationFrequency()
{
			...
			for (int i = 0; i < FILEWRITER_BUFFER_SIZE; i++)
			{
				...
				for (int m = 1; m <= numRows; m++)
				{
					for (int n = 1; n <= numCols; n++)
					{
						...
						{
							// 最后一行不加逗号
							outputFile << "\t\t" << R"("(re)Sink_Data_)" << m << n << R"(":)" << m_pdBuffer[i](m - 1, n - 1).real() << "," << "\r\n";
							outputFile << "\t\t" << R"("(im)Sink_Data_)" << m << n << R"(":)" << m_pdBuffer[i](m - 1, n - 1).imag() << "\r\n";
						}
						else
						{
							outputFile << "\t\t" << R"("(re)Sink_Data_)" << m << n << R"(":)" << m_pdBuffer[i](m - 1, n - 1).real() << "," << "\r\n";
