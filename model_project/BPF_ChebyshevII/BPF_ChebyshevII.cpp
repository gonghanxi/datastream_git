...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE ( BPF_ChebyshevII )
{	
	SET_MODEL_DESCRIPTION("Bandpass Chebyshev II Filter");
	SET_MODEL_SYMBOL("SYM_BPF");
	...
	{
		SystemVueModelBuilder::DFParam param = ADD_MODEL_PARAMETER(PassBandwidth);
		param.SetDescription("Passband bandwidth");
		param.SetUnit(SystemVueModelBuilder::Units::FREQUENCY);
...

	{
		...
		param.SetDescription("Passband ripple in dB");
		param.SetUnit(SystemVueModelBuilder::Units::NONE);
	...
	{
		SystemVueModelBuilder::DFParam param = ADD_MODEL_PARAMETER(StopBandwidth);
		param.SetDescription("Stopband bandwidth (required only when OrderType = Auto)");
		param.SetUnit(SystemVueModelBuilder::Units::FREQUENCY);
		param.SetDefaultValue("100e3");
	}

	{
		SystemVueModelBuilder::DFParam param = ADD_MODEL_PARAMETER(StopRipple);
...
BPF_ChebyshevII::BPF_ChebyshevII()
{

}

ERESULT BPF_ChebyshevII::PropagateCharacterizationFrequency()
{
	bool bStatus = true;
...
bool BPF_ChebyshevII::Setup()
{
	...
	// 滤波器初始化
	double	CutoffFreq = StopBandwidth / 2;

	if (SampleRate > StopBandwidth)
	{
		shelfFilterReal.setup(FilterOrder, SampleRate, CutoffFreq, StopRipple);
		shelfFilterImag.setup(FilterOrder, SampleRate, CutoffFreq, StopRipple);
	}

	else if (SampleRate <= StopBandwidth)
	{
		// 欠采样时全通输出
		if (UnderSampledModel == ModelAsAllpass)
		{
            char str[256];
            sprintf(str, "Model as all-pass in the under-sampling situation. SampleRate (%f) should be larger than StopBandwidth (%f).", SampleRate, StopBandwidth);
			POST_WARNING(str);
            LOG_WARN("Model as all-pass in the under-sampling situation. SampleRate (",SampleRate,") should be larger than PassBandwidth (",PassBandwidth,").");
		}
		// 欠采样时报错
		else if (UnderSampledModel == ErrorOut)
		{
            char str[256];
            sprintf(str, "SampleRate (%f) should be larger than StopBandwidth (%f).", SampleRate, StopBandwidth);
			POST_ERROR(str);
            LOG_ERROR("SampleRate (",SampleRate,") should be larger than PassBandwidth (",PassBandwidth,").");
			bStatus = false;
		}
	}
	return bStatus;
...
// 计算 exp(j * 2 * PI * f_c * t)
std::complex<double> BPF_ChebyshevII::complexExponential(double f_c, double t) {
	// 计算角频率：ω = 2 * π * f_c
...
// 分贝转倍数
double BPF_ChebyshevII::dBToPowerRatio(double dB) {
...
bool BPF_ChebyshevII::Run()
{
	if (SampleRate > StopBandwidth)
	{
		...
		std::complex<double> exp = complexExponential(fc - FCenter, t);
		...
		output[0] = zz * complexExponential(FCenter - fc, t);
	}
	else if (SampleRate <= StopBandwidth)
	{
		output[0] = input[0];
