...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE ( SinkInt )
{	
...
bool SinkInt::Setup()
{
...

	{
        ...
        unsigned long iMaxCollect = (unsigned(1) << 31) / sizeof(int);
	...
	if (m_control.CollectData()) // Check if we should still collect data
	{
		...
		if (m_iBuffer == FILEWRITER_BUFFER_SIZE)
		{
			...
			for (int i = 0; i < FILEWRITER_BUFFER_SIZE; i++)
			{
				...
				outputFile << "\t\t" << R"("Sink_Data": )" << m_pdBuffer[i] << "\r\n";
				outputFile << "\t}," << "\r\n";
...
bool SinkInt::Finalize()
{
	...
	outputFile.seekp(-4, std::ios::end);	// 如果是CRLF换行则填-4
	outputFile << "\r\n" << "]" << "\r\n";
