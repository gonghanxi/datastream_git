...
SinkInt_Block::SinkInt_Block(const std::string &name)
    :Block(name)
{
    Index = 1;
    m_sampleRate = 0;
...
SinkInt_Block::~SinkInt_Block()
{
    cleanup();
}

bool SinkInt_Block::Setup()
{
    Block::Setup();
    ...
    case SinkInt::Samples:
        totalSamples = m_SampleStop - m_SampleStart + 1;
        break;
    }

    // Windows 2GB 文件大小限制
    unsigned long maxSamples = (static_cast<unsigned long>(1) << 31) / sizeof(int);
...
bool SinkInt_Block::Initialize()
{
    SetBlockType(Block::BlockType::SINK);

    m_sink = std::make_unique<SinkInt>();

    AddInputPort("input", m_sink->input, 1, DataType::TIMED_INT);
    ...
    else if(m_StartStopOption == SinkInt::Time) {
...
bool SinkInt_Block::Done()
{
    qDebug() << "[SinkInt_Block] Done - 处理剩余数据"
...
bool SinkInt_Block::IsCollectionComplete()
{
...
void SinkInt_Block::SetParameters(int SampleStart, int SampleStop, double TimeStart, double TimeStop, SinkInt::SelectedStartStopOption StartStopOption, char *Filename)
{
...
SinkInt::SelectedStartStopOption SinkInt_Block::ConvertStringToSelected(const std::string &value)
{
    // 去除字符串前后的空格
    ...
    if (lowerValue == "auto" || lowerValue == "0") {
        ...
        return SinkInt::Samples;
    } else if (lowerValue == "time" || lowerValue == "2") {
        return SinkInt::Time;
    }
}

void SinkInt_Block::CopyStringToCharPtr(const std::string &src, char *&dest)
{
    delete[] dest;
    if (!src.empty()) {
...
char* SinkInt_Block::CopyStringToCharPtr(const std::string& src) {
...
char* SinkInt_Block::combinePathWithJsonSuffix(const fs::path& linkKeyFolder, const char* fileName) {
...
void SinkInt_Block::SetDefaultParameters()
{
    m_StartStopOption = SinkInt::Auto;
    m_SampleStart = 0;
    m_SampleStop = 1;
    m_TimeStart = 0.0;
    m_TimeStop = 1.0;
...
// ---------- 文件操作 ----------
bool SinkInt_Block::openFileForWrite()
{
    m_qfile.setFileName(m_fullPath);
    if (!m_qfile.open(QIODevice::WriteOnly | QIODevice::Text)) {
        LOG_INFO("[SinkInt_Block] 无法创建文件:", m_qfile.errorString().toStdString());
        return false;
    }
    ...
    qDebug() << "[SinkInt_Block] 文件已创建:" << m_fullPath;
    return true;
}

bool SinkInt_Block::openFileForAppend()
{
...
void SinkInt_Block::closeFileProperly()
{
...
void SinkInt_Block::writeDataPointToStream(size_t bufferIndex, unsigned long long dataIndex)
{
...
void SinkInt_Block::RunDealData()
{
...
// ---------- 时间驱动中途刷新 ----------
void SinkInt_Block::flushToFile()
{
...
bool SinkInt_Block::isTimeDrivenMode() const { return m_isTimeDrivenMode; }
void SinkInt_Block::setTimeDrivenMode(bool enabled) { m_isTimeDrivenMode = enabled; }
double SinkInt_Block::GetCurrentSimulationTime() const { return m_currentSimulationTime; }
