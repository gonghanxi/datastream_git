...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE(RADAR_Antenna_Tx)
{
	SET_MODEL_DESCRIPTION("Transmitting Antenna");
	SET_MODEL_SYMBOL("SYM_RADAR_Antenna_Tx@RADAR Symbols");
...
RADAR_Antenna_Tx::RADAR_Antenna_Tx()
...
ERESULT RADAR_Antenna_Tx::PropagateCharacterizationFrequency()
{
...
int RADAR_Antenna_Tx::determineTargetCount()
{
...
bool RADAR_Antenna_Tx::hasBeamAzimuthPort()
{
	return BeamAzimuth.IsConnected();
}


bool RADAR_Antenna_Tx::hasBeamElevationPort()
{
	return BeamElevation.IsConnected();
}


bool RADAR_Antenna_Tx::hasTargetAzimuthPort(int ch)
{
	if (ch < 0)
	{
		return false;
...
bool RADAR_Antenna_Tx::hasTargetElevationPort(int ch)
{
	if (ch < 0)
	{
		return false;
...
double RADAR_Antenna_Tx::getArrayValue(const double* data,
	int size,
	int index,
...
void RADAR_Antenna_Tx::getBeamAngle(double timeNow,
	double& beamAzRad,
	double& beamElRad)
{
...
double RADAR_Antenna_Tx::getCircularScanAzimuth(double timeNow) const
{
	// ScanRate 单位：rpm
...
double RADAR_Antenna_Tx::getSectorScanAzimuth(double timeNow,
...
void RADAR_Antenna_Tx::getRasterScanAngle(double timeNow,
	bool bidirectional,
	double& azDeg,
	double& elDeg) const
{
...
double RADAR_Antenna_Tx::calcAntennaAmplitudeGain(double targetAzRad,
	double targetElRad,
	double beamAzRad,
	double beamElRad,
	double fcHz) const
{
	...
	{
		return calcUserPatternGain(targetAzRad,
			targetElRad,
			beamAzRad,
			beamElRad);
	}
	...
	// 关键修正：
	// Tx 使用发射天线电压/包络孔径增益：
	// G_tx_amp = sqrt(4*pi*A) / lambda
	// 不使用 Rx 曾拟合过的 sqrt(4*pi)*H*W/lambda^2。
	const double apertureAmpGain =
		std::sqrt(4.0 * M_PI * apertureArea) / lambda;
...
double RADAR_Antenna_Tx::calcUserPatternGain(double targetAzRad,
	double targetElRad,
	double beamAzRad,
...
double RADAR_Antenna_Tx::calcAnalyticPatternFactor(double dAzRad,
	double dElRad,
	double lambda) const
{
	if (lambda <= 0.0)
...
double RADAR_Antenna_Tx::angularSeparation(double az1,
	double el1,
	double az2,
	double el2) const
{
...
double RADAR_Antenna_Tx::deg2rad(double x)
{
	return x * M_PI / 180.0;
}


double RADAR_Antenna_Tx::rad2deg(double x)
{
	return x * 180.0 / M_PI;
}


double RADAR_Antenna_Tx::normalizeRad(double x)
{
	while (x > M_PI)
	{
		x -= 2.0 * M_PI;
...
double RADAR_Antenna_Tx::wrap360(double x)
{
...
double RADAR_Antenna_Tx::sinc(double x)
{
	if (std::fabs(x) < 1.0e-12)
	{
		return 1.0;
...
double RADAR_Antenna_Tx::besselI0(double x)
{
	// 简单级数近似，当前版本保留备用。
	double sum = 1.0;
	double term = 1.0;
