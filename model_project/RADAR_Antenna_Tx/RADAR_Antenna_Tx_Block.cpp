#include "RADAR_Antenna_Tx_Block.h"
...
bool RADAR_Antenna_Tx_Block::parseArrayString(const std::string& arrayStr, std::vector<double>& outArray)
{
    outArray.clear();
...
RADAR_Antenna_Tx_Block::RADAR_Antenna_Tx_Block(const std::string& name)
    : Block(name)
{
}

void RADAR_Antenna_Tx_Block::SetDefaultParamters()
{
...
void RADAR_Antenna_Tx_Block::SetParameters()
{
...
bool RADAR_Antenna_Tx_Block::Setup()
{
    Block::Setup();
...
bool RADAR_Antenna_Tx_Block::Run()
{
...
bool RADAR_Antenna_Tx_Block::Initialize()
{
...
bool RADAR_Antenna_Tx_Block::DataStreamRun()
{
    ...
    // 确定目标通道数：从 TargetAzimuth / TargetElevation 端口或参数数组推导
    ...
    for (int ch = 0; ch < targetCount; ++ch) {
        ...
        // 端口连接则读端口（单位 rad），否则用参数（单位 deg → rad）
...
bool RADAR_Antenna_Tx_Block::TimeDrivenRun()
{
    // ① 累积输入
    {
        auto inputData = ReadInputData<EnvelopeSignal>(GetInputPortName(4));
...
RADAR_Antenna_Tx_Block::SelectedRadarWorkMode
RADAR_Antenna_Tx_Block::ConvertStringToRadarWorkMode(const std::string& value)
{
...
RADAR_Antenna_Tx_Block::SelectedPattern
RADAR_Antenna_Tx_Block::ConvertStringToPattern(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
...
RADAR_Antenna_Tx_Block::SelectedAntennaScanPattern
RADAR_Antenna_Tx_Block::ConvertStringToAntennaScanPattern(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
...
double RADAR_Antenna_Tx_Block::deg2rad(double x) { return x * M_PI / 180.0; }
double RADAR_Antenna_Tx_Block::rad2deg(double x) { return x * 180.0 / M_PI; }

double RADAR_Antenna_Tx_Block::normalizeRad(double x)
{
...
double RADAR_Antenna_Tx_Block::wrap360(double x)
{
...
double RADAR_Antenna_Tx_Block::sinc(double x)
{
    if (std::fabs(x) < 1.0e-12) return 1.0;
    return std::sin(x) / x;
}

double RADAR_Antenna_Tx_Block::besselI0(double x)
{
...
double RADAR_Antenna_Tx_Block::getArrayValue(const double* data, int size, int index, double defaultValue) const
{
...
void RADAR_Antenna_Tx_Block::getBeamAngle(double timeNow, double& beamAzRad, double& beamElRad)
{
    // Beam 端口连接时，端口单位为 rad。
    // Beam 参数单位为 deg。
    // （在 Block 层，连接判断已由 ReadInputData 完成，这里仅按规则计算备选值）
    beamAzRad = 0.0;
    beamElRad = 0.0;
...
double RADAR_Antenna_Tx_Block::getCircularScanAzimuth(double timeNow) const
{
...
double RADAR_Antenna_Tx_Block::getSectorScanAzimuth(double timeNow, bool bidirectional) const
{
...
void RADAR_Antenna_Tx_Block::getRasterScanAngle(double timeNow, bool bidirectional,
...
double RADAR_Antenna_Tx_Block::angularSeparation(double az1, double el1,
...
double RADAR_Antenna_Tx_Block::calcAntennaAmplitudeGain(double targetAzRad, double targetElRad,
                                                         ...
                                                         double fcHz) const
{
    ...
    if (m_Pattern == SelectedPattern::UserDefinedPattern) {
        return calcUserPatternGain(targetAzRad, targetElRad, beamAzRad, beamElRad);
    }
    ...
    const double apertureAmpGain = std::sqrt(4.0 * M_PI * apertureArea) / lambda;
...
double RADAR_Antenna_Tx_Block::calcUserPatternGain(double targetAzRad, double targetElRad,
...
double RADAR_Antenna_Tx_Block::calcAnalyticPatternFactor(double dAzRad, double dElRad,
                                                          double lambda) const
{
    if (lambda <= 0.0) { return 0.0; }
