#ifndef SINKCX_M_BLOCK_H
#define SINKCX_M_BLOCK_H
#include "SinkCx_M.h"
#include "Block.h"
...
class SYSTEMVUEMODELBUILDER_API SinkCx_M_Block : public Block
{
public:
    SinkCx_M_Block(const std::string& name);
    ~SinkCx_M_Block();
...
private:
    ...
    SinkCx_M::SelectedStartStopOption ConvertStringToSelected(const std::string& value);
    ...
    std::unique_ptr<SinkCx_M> m_sink;

    SinkCx_M::SelectedStartStopOption m_StartStopOption;
    int m_SampleStart;
    int m_SampleStop;
    double m_TimeStart;
    double m_TimeStop;
...
RegAlgo(SinkCx_M_Block);

#endif // SINKCX_M_BLOCK_H
