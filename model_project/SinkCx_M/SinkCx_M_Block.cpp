#include "SinkCx_M_Block.h"
#include <QDir>

#define FILEWRITER_BUFFER_SIZE 1000000

SinkCx_M_Block::SinkCx_M_Block(const std::string &name)
    :Block(name)
{
    Index = 1;
    m_sampleRate = 0;
...
SinkCx_M_Block::~SinkCx_M_Block()
{
    cleanup();
}

bool SinkCx_M_Block::Setup()
{
    Block::Setup();
...
bool SinkCx_M_Block::Run()
{
    ...
    // 处理变长数据（TIMED_DCOMPLEX_MATRIX）或普通 DOUBLE
    // 读取一个或多个数据点
    // 对于每个数据点：
    ...
    // 处理单个 DCOMPLEX_MATRIX 数据
    else {
//        DComplexMatrix inputData;
...
bool SinkCx_M_Block::Initialize()
{
    SetBlockType(Block::BlockType::SINK);

    m_sink = std::make_unique<SinkCx_M>();

    AddInputPort("input", m_sink->input, 1, DataType::MATRIX_TIME_DCOMPLEX);
    ...
    else if(m_StartStopOption == SinkCx_M::Time) {
...
bool SinkCx_M_Block::Done()
{
    qDebug() << "[SinkCx_M_Block] Done - 处理剩余数据"
...
bool SinkCx_M_Block::IsCollectionComplete()
{
...
void SinkCx_M_Block::SetParameters()
{
    if(m_sink) {
...
SinkCx_M::SelectedStartStopOption SinkCx_M_Block::ConvertStringToSelected(const std::string &value)
{
    // 去除字符串前后的空格
    ...
    if (lowerValue == "auto" || lowerValue == "0") {
        return SinkCx_M::Auto;
    } else if (lowerValue == "samples" || lowerValue == "1") {
        return SinkCx_M::Samples;
    } else if (lowerValue == "time" || lowerValue == "2") {
        return SinkCx_M::Time;
    }
}

void SinkCx_M_Block::CopyStringToCharPtr(const std::string &src, char *&dest)
{
    delete[] dest;
    if (!src.empty()) {
...
char* SinkCx_M_Block::CopyStringToCharPtr(const std::string& src) {
...
char* SinkCx_M_Block::combinePathWithJsonSuffix(const fs::path& linkKeyFolder, const char* fileName) {
...
void SinkCx_M_Block::SetDefaultParameters()
{
    m_StartStopOption = SinkCx_M::Auto;
    m_SampleStart = 0;
    m_SampleStop = 1;
    m_TimeStart = 0.0;
    m_TimeStop = 1.0;
...
// ---------- 文件操作 ----------
bool SinkCx_M_Block::openFileForWrite()
{
    m_qfile.setFileName(m_fullPath);
    if (!m_qfile.open(QIODevice::WriteOnly | QIODevice::Text)) {
        LOG_INFO("[SinkCx_M_Block] 无法创建文件:", m_qfile.errorString().toStdString());
        return false;
    }
    ...
    qDebug() << "[SinkCx_M_Block] 文件已创建:" << m_fullPath;
    return true;
}

bool SinkCx_M_Block::openFileForAppend()
{
...
void SinkCx_M_Block::closeFileProperly()
{
...
void SinkCx_M_Block::writeDataPointToStream(size_t bufferIndex, unsigned long long dataIndex)
{
...
void SinkCx_M_Block::RunDealData()
{
...
// ---------- 时间驱动中途刷新 ----------
void SinkCx_M_Block::flushToFile()
{
...
bool SinkCx_M_Block::isTimeDrivenMode() const { return m_isTimeDrivenMode; }
void SinkCx_M_Block::setTimeDrivenMode(bool enabled) { m_isTimeDrivenMode = enabled; }
double SinkCx_M_Block::GetCurrentSimulationTime() const { return m_currentSimulationTime; }
