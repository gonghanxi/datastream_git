...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE ( LPF_ChebyshevII )
{	
	SET_MODEL_DESCRIPTION("Lowpass Chebyshev II Filter");
	SET_MODEL_SYMBOL("SYM_LPF");
...
LPF_ChebyshevII::LPF_ChebyshevII()
{

}

ERESULT LPF_ChebyshevII::PropagateCharacterizationFrequency()
{
	bool bStatus = true;
...
bool LPF_ChebyshevII::Setup()
{
	...
	if (SampleRate > 2 * StopFreq)
	{
		// 滤波器初始化
		shelfFilterReal.setup(FilterOrder, SampleRate, StopFreq, StopRipple);
		shelfFilterImag.setup(FilterOrder, SampleRate, StopFreq, StopRipple);
	}

	else if (SampleRate <= 2 * StopFreq)
	{
		// 欠采样时全通输出
		if (UnderSampledModel == ModelAsAllpass)
		{
            char str[256];
            sprintf(str, "Model as all-pass in the under-sampling situation. SampleRate (%f) should be larger than 2 * StopFreq (%f).", SampleRate, 2 * StopFreq);
			POST_WARNING(str);
            LOG_INFO("Model as all-pass in the under-sampling situation. SampleRate (",SampleRate,") should be larger than 2 * PassFreq (",2 * PassFreq,").");
		}
		// 欠采样时报错
		else if (UnderSampledModel == ErrorOut)
		{
            char str[256];
            sprintf(str, "SampleRate (%f) should be larger than 2 * StopFreq (%f).", SampleRate, 2 * StopFreq);
			POST_ERROR(str);
...
// 计算 exp(j * 2 * PI * f_c * t)
std::complex<double> LPF_ChebyshevII::complexExponential(double f_c, double t) {
	// 计算角频率：ω = 2 * π * f_c
...
// 分贝转倍数
double LPF_ChebyshevII::dBToPowerRatio(double dB) {
