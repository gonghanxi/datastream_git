#include "RADAR_PhasedArrayTx_Block.h"
...
RADAR_PhasedArrayTx_Block::RADAR_PhasedArrayTx_Block(const std::string& name)
    : Block(name)
    , m_NumChannels(0)
{
}
...
void RADAR_PhasedArrayTx_Block::SetDefaultParameters()
{
    m_Configuration      = RADAR_PhasedArrayTx::UniformLinearArray;
    m_AxisType           = RADAR_PhasedArrayTx::X;
    m_Array2DShapeType   = RADAR_PhasedArrayTx::Full;
    m_NumOfAnt1D         = 8;
    m_NumOfAnt2D_H       = 8;
    m_NumOfAnt2D_V       = 8;
    m_ElementFactor      = 1.0;
    m_SpaceType          = RADAR_PhasedArrayTx::Uniform;
    m_GridType           = RADAR_PhasedArrayTx::Rectangular;
    m_D                  = 0.5;
    m_D_H                = 0.5;
    m_D_V                = 0.5;
    ...
    m_ReliabilityType    = RADAR_PhasedArrayTx::NoFailures;
    m_FailureProbability = 0.1;
    m_TargetTheta        = 0.0;
    m_TargetPhi          = 0.0;
    m_WindowType         = RADAR_PhasedArrayTx::Rectangle;
    m_KaiserWindowParameter = 1.0;
    m_Sidelobe_Levels    = -20.0;
    m_nBar               = 2;
    m_IsPhaseShift       = RADAR_PhasedArrayTx::Yes;
    m_BeamTheta          = 0.0;
    m_BeamPhi            = 0.0;
    m_QuantizationType   = RADAR_PhasedArrayTx::No;
    m_PhaseShifterBitwidth = 5;
    m_PhaseShiftType     = RADAR_PhasedArrayTx::CalculateByThetaAndPhi;
...
void RADAR_PhasedArrayTx_Block::SetParameters()
{
    if (!m_algo) return;
...
bool RADAR_PhasedArrayTx_Block::Setup()
{
    Block::Setup();
...
bool RADAR_PhasedArrayTx_Block::Run()
{
...
bool RADAR_PhasedArrayTx_Block::DataStreamRun()
{
...
bool RADAR_PhasedArrayTx_Block::TimeDrivenRun()
{
    if (m_NumChannels <= 0) return true;
...
bool RADAR_PhasedArrayTx_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_algo = std::make_unique<RADAR_PhasedArrayTx>();
    ...
    // ---- 参数校验 (from RADAR_PhasedArrayTx::Setup) ----
    bool bStatus = true;
    if (m_NumOfAnt1D <= 0 && m_Configuration == RADAR_PhasedArrayTx::UniformLinearArray)
    {
        LOG_ERROR("NumOfAnt1D must be > 0");
        bStatus = false;
    }
    if (m_NumOfAnt2D_H <= 0 && m_Configuration == RADAR_PhasedArrayTx::UniformRectangularArray)
    {
        LOG_ERROR("NumOfAnt2D_H must be > 0");
        bStatus = false;
    }
    if (m_NumOfAnt2D_V <= 0 && m_Configuration == RADAR_PhasedArrayTx::UniformRectangularArray)
    {
        LOG_ERROR("NumOfAnt2D_V must be > 0");
        bStatus = false;
    }
    if (m_D <= 0 && m_Configuration == RADAR_PhasedArrayTx::UniformLinearArray && m_SpaceType == RADAR_PhasedArrayTx::Uniform)
    {
        LOG_ERROR("D must be > 0");
        bStatus = false;
    }
    if (m_D_H <= 0 && m_Configuration == RADAR_PhasedArrayTx::UniformRectangularArray && m_SpaceType == RADAR_PhasedArrayTx::Uniform)
    {
        LOG_ERROR("D_H must be > 0");
        bStatus = false;
    }
    if (m_D_V <= 0 && m_Configuration == RADAR_PhasedArrayTx::UniformRectangularArray && m_SpaceType == RADAR_PhasedArrayTx::Uniform)
    {
        LOG_ERROR("D_V must be > 0");
        bStatus = false;
    }
    if ((m_FailureProbability < 0 || m_FailureProbability > 1) && m_ReliabilityType == RADAR_PhasedArrayTx::RandomElement)
    {
...
RADAR_PhasedArrayTx::SelectedConfiguration RADAR_PhasedArrayTx_Block::ConvertStringToConfiguration(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "uniformlineararray"       || lower == "0") return RADAR_PhasedArrayTx::UniformLinearArray;
    if (lower == "uniformrectangulararray"  || lower == "1") return RADAR_PhasedArrayTx::UniformRectangularArray;
    return RADAR_PhasedArrayTx::UniformLinearArray;
}
...
RADAR_PhasedArrayTx::SelectedAxisType RADAR_PhasedArrayTx_Block::ConvertStringToAxisType(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "x" || lower == "0") return RADAR_PhasedArrayTx::X;
    if (lower == "y" || lower == "1") return RADAR_PhasedArrayTx::Y;
    if (lower == "z" || lower == "2") return RADAR_PhasedArrayTx::Z;
    return RADAR_PhasedArrayTx::X;
}
...
RADAR_PhasedArrayTx::SelectedArray2DShapeType RADAR_PhasedArrayTx_Block::ConvertStringToArray2DShapeType(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "full"       || lower == "0") return RADAR_PhasedArrayTx::Full;
    if (lower == "customized" || lower == "1") return RADAR_PhasedArrayTx::Customized;
    return RADAR_PhasedArrayTx::Full;
}
...
RADAR_PhasedArrayTx::SelectedSpaceType RADAR_PhasedArrayTx_Block::ConvertStringToSpaceType(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "uniform"    || lower == "0") return RADAR_PhasedArrayTx::Uniform;
    if (lower == "nonuniform" || lower == "1") return RADAR_PhasedArrayTx::NonUniform;
    return RADAR_PhasedArrayTx::Uniform;
}
...
RADAR_PhasedArrayTx::SelectedGridType RADAR_PhasedArrayTx_Block::ConvertStringToGridType(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "rectangular" || lower == "0") return RADAR_PhasedArrayTx::Rectangular;
    if (lower == "triangular"  || lower == "1") return RADAR_PhasedArrayTx::Triangular;
    return RADAR_PhasedArrayTx::Rectangular;
}
...
RADAR_PhasedArrayTx::SelectedReliabilityType RADAR_PhasedArrayTx_Block::ConvertStringToReliabilityType(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "nofailures"     || lower == "0") return RADAR_PhasedArrayTx::NoFailures;
    if (lower == "randomelement"  || lower == "1") return RADAR_PhasedArrayTx::RandomElement;
    return RADAR_PhasedArrayTx::NoFailures;
}
...
RADAR_PhasedArrayTx::SelectedWindowType RADAR_PhasedArrayTx_Block::ConvertStringToWindowType(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "rectangle"     || lower == "0") return RADAR_PhasedArrayTx::Rectangle;
    if (lower == "bartlett"      || lower == "1") return RADAR_PhasedArrayTx::Bartlett;
    if (lower == "hanning"       || lower == "2") return RADAR_PhasedArrayTx::Hanning;
    if (lower == "hamming"       || lower == "3") return RADAR_PhasedArrayTx::Hamming;
    if (lower == "blackman"      || lower == "4") return RADAR_PhasedArrayTx::Blackman;
    if (lower == "steepblackman" || lower == "5") return RADAR_PhasedArrayTx::SteepBlackman;
    if (lower == "kaiser"        || lower == "6") return RADAR_PhasedArrayTx::Kaiser;
    if (lower == "taylor"        || lower == "7") return RADAR_PhasedArrayTx::Taylor;
    return RADAR_PhasedArrayTx::Rectangle;
}
...
RADAR_PhasedArrayTx::SelectedYesorNo RADAR_PhasedArrayTx_Block::ConvertStringToYesorNo(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "yes" || lower == "0") return RADAR_PhasedArrayTx::Yes;
    if (lower == "no"  || lower == "1") return RADAR_PhasedArrayTx::No;
    return RADAR_PhasedArrayTx::Yes;
}
...
RADAR_PhasedArrayTx::SelectedPhaseShiftType RADAR_PhasedArrayTx_Block::ConvertStringToPhaseShiftType(const std::string& value)
{
    const std::string lower = ToLowerCopy(TrimCopy(value));
    if (lower == "calculatebythetaandphi" || lower == "0") return RADAR_PhasedArrayTx::CalculateByThetaAndPhi;
    if (lower == "desiredphaseshift"      || lower == "1") return RADAR_PhasedArrayTx::DesiredPhaseShift;
    return RADAR_PhasedArrayTx::CalculateByThetaAndPhi;
}
