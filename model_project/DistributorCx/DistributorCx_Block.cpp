#include "DistributorCx_Block.h"

DistributorCx_Block::DistributorCx_Block(const std::string &name)
    :Block(name)
{

}
bool DistributorCx_Block::Setup()
{
    Block::Setup();
...
bool DistributorCx_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_Distributor = std::make_unique<DistributorCx>();
    ...
    AddInputPort("input", m_Distributor->input, 1, DataType::CIRCULAR_BUFFER_DCOMPLEX);
    AddOutputPort("output", m_Distributor->output, 1, DataType::DCOMPLEX_BUS);
    return true;
}

void DistributorCx_Block::SetParameters()
{
    if(!m_Distributor) return;
    m_Distributor->BlockSize = m_BlockSize;
}

void DistributorCx_Block::SetDefaultParameters()
{
    m_BlockSize = 1;
}

bool DistributorCx_Block::ModelsSetup()
{
