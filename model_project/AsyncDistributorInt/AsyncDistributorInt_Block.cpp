#include "AsyncDistributorInt_Block.h"

AsyncDistributorInt_Block::AsyncDistributorInt_Block(const std::string &name)
    :Block(name)
{

}
bool AsyncDistributorInt_Block::Setup()
{
    Block::Setup();
...
bool AsyncDistributorInt_Block::Run()
{
    if(IsVariableStepMode()) return TimeDrivenRun();
    return DataStreamRun();
}

bool AsyncDistributorInt_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_AsyncDistributor = std::make_unique<AsyncDistributorInt>();
    ...
    AddInputPort("input", m_AsyncDistributor->input, 1, DataType::CIRCULAR_BUFFER_INT);
    AddOutputPort("output", m_AsyncDistributor->output, 1, DataType::INT_BUS);
    return true;
}

void AsyncDistributorInt_Block::SetParameters()
{
    if(!m_AsyncDistributor) return;
    m_AsyncDistributor->BlockSizes = m_BlockSizes;
}

void AsyncDistributorInt_Block::SetDefaultParameters()
{
    m_BlockSizes.Resize(1,1);
    m_BlockSizes(0,0) = 1;
}

bool AsyncDistributorInt_Block::ModelsSetup()
{
...
bool AsyncDistributorInt_Block::DataStreamRun()
{
...
bool AsyncDistributorInt_Block::TimeDrivenRun()
{
    std::string inputPort = GetInputPortName(0);
    std::string outputPort = GetOutputPortName(0);
    ...
    for (size_t i = 0; i < m_channelQueues.size(); ++i) {
        if (!m_channelQueues[i].empty()) {
            int sample = m_channelQueues[i].front();
            m_channelQueues[i].pop();
            // 写入单个样本到对应通道
            outputBuffer->WriteDataToChannel(static_cast<int>(i), std::vector<int>{sample});
            anyOutput = true;
