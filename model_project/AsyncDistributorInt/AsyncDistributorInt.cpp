...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE(AsyncDistributorInt)
{
...
AsyncDistributorInt::AsyncDistributorInt()
	: BlockSizes(1, 1)
{
	BlockSizes(0) = 1;
}

bool AsyncDistributorInt::Setup()
{
	...
	if (numBlockVals == 0)
	{
		POST_ERROR("AsyncDistributorInt: BlockSizes must contain at least one element.");
		return false;
	}
	...
	if (numBlockVals == 1)
	{
		...
		if (B <= 0)
		{
			POST_ERROR("AsyncDistributorInt: elements of BlockSizes array must all be > 0.");
			return false;
		}
		...
		std::stringstream ss;
		ss << "AsyncDistributorInt: BlockSizes has 1 element, applying this value to all "
			<< numOutputs << " output(s).";
	...
	else if (numBlockVals >= numOutputs)
	{
		...
		{
			std::stringstream ss;
			ss << "AsyncDistributorInt: BlockSizes has "
	...
	else 
	{
		...
		std::stringstream ss;
		ss << "AsyncDistributorInt: BlockSizes has " << numBlockVals
	...
	for (size_t i = 0; i < numOutputs; ++i)
	{
		...
		if (Bi <= 0)
		{
			POST_ERROR("AsyncDistributorInt: expanded BlockSizes element is not > 0.");
			return false;
		}
	...
	if (numInputData == 0)
	{
		POST_ERROR("AsyncDistributorInt: total number of input samples per run must be > 0.");
		return false;
	}
