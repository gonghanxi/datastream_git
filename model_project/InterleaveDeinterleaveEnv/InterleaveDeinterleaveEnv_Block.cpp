#include "InterleaveDeinterleaveEnv_Block.h"

InterleaveDeinterleaveEnv_Block::InterleaveDeinterleaveEnv_Block(const std::string& name)
    :Block(name)
{

}
bool InterleaveDeinterleaveEnv_Block::Setup()
{
    Block::Setup();
...
bool InterleaveDeinterleaveEnv_Block::Run()
{
    if(IsVariableStepMode()) return TimeDrivenRun();
    return DataStreamRun();
}

bool InterleaveDeinterleaveEnv_Block::DataStreamRun()
{
    // 获取端口名称
    ...
    std::vector<EnvelopeSignal> outputData(outputRate);
...
bool InterleaveDeinterleaveEnv_Block::TimeDrivenRun()
{
    // 获取端口名称
    size_t outputRate = m_inter->output.GetRate();
    ...
    if(m_inputBuffer.size() >= static_cast<size_t>(m_inter->m_blockSize)) {
        std::vector<EnvelopeSignal> outputData(outputRate);
        ...
        if (!m_outputQueue.empty())
        {
            ...
            qDebug() << "[InterleaveDeinterleaveEnv_Block] 分发输出:" << m_outputCount
...
bool InterleaveDeinterleaveEnv_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_inter = std::make_unique<InterleaveDeinterleaveEnv>();
    ...
    AddInputPort("input", m_inter->input, static_cast<size_t>(m_inter->m_blockSize), Block::DataType::ENVELOPE_SIGNAL);
    AddOutputPort("output", m_inter->output, static_cast<size_t>(m_inter->m_blockSize), Block::DataType::ENVELOPE_SIGNAL);

    return true;
}

void InterleaveDeinterleaveEnv_Block::SetParameters()
{
    if(!m_inter) return;
    m_inter->Rows = Rows;
...
bool InterleaveDeinterleaveEnv_Block::ModelSetup()
{
    bool bStatus = true;
    if (Rows < 1)
    {
    ...
    if (Rows > 0 && Columns > 0)
    {
        ...
        m_inter->m_blockSize = static_cast<unsigned>(N);

        m_inter->input.SetRate(m_inter->m_blockSize);
        m_inter->output.SetRate(m_inter->m_blockSize);
    }
    return bStatus;
}

void InterleaveDeinterleaveEnv_Block::SetDefaultParameters()
{
    Rows = 8;
    Columns = 8;
}
