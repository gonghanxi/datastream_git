...
bool UpSampleEnv_Block::DataStreamRun()
{
    // 获取输入输出端口名称
...
bool UpSampleEnv_Block::TimeDrivenRun()
{
    // 获取输入输出端口名称
    BufferReader* inputport = GetInputPort(GetInputPortName(0));
    std::string outputPortName = GetOutputPortName(0);
    ...
    if(m_inputBuffer.size() >= 1) {
        ...
        if(m_mode == UpSampleEnv::Insertzeros) {
            for(size_t i = 0; i < inLen; i++) {
                SystemVueModelBuilder::EnvelopeSignal InputSample = m_inputBuffer[i];
                outputData.push_back(InputSample);
                for(int j = 1; j < m_factor; j++) {
                    outputData.push_back(0);
                }
            ...
            // 线性插值模式
            for(size_t i = 0; i < inLen; i++) {
                SystemVueModelBuilder::EnvelopeSignal currentSample = m_inputBuffer[i];
                ...
                if(i == 0) {
                    ...
                    // 处理后续输入点
                    SystemVueModelBuilder::EnvelopeSignal prevSample = m_inputBuffer[i-1];
        ...
        if (!m_outputQueue.empty()) {
            ...
            qDebug() << "[UpSampleEnv_Block] 分发输出:" << m_outputCount
