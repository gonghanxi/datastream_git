#include "Sink_M_Block.h"
#include <QDir>

#define FILEWRITER_BUFFER_SIZE 1000000

Sink_M_Block::Sink_M_Block(const std::string &name)
    :Block(name)
{
    Index = 1;
    m_sampleRate = 0;
...
bool Sink_M_Block::Initialize()
{
    SetBlockType(Block::BlockType::SINK);

    m_sink = std::make_unique<Sink_M>();
    ...
    else if(m_StartStopOption == Sink_M::Time) {
...
Sink_M::SelectedStartStopOption Sink_M_Block::ConvertStringToSelected(const std::string &value)
{
    // 去除字符串前后的空格
    ...
    if (lowerValue == "auto" || lowerValue == "0") {
        ...
        return Sink_M::Samples;
    } else if (lowerValue == "time" || lowerValue == "2") {
        return Sink_M::Time;
    }
}

void Sink_M_Block::CopyStringToCharPtr(const std::string &src, char *&dest)
{
    delete[] dest;
    if (!src.empty()) {
...
char* Sink_M_Block::CopyStringToCharPtr(const std::string& src) {
...
void Sink_M_Block::SetDefaultParameters()
{
    m_StartStopOption = Sink_M::Auto;
    m_SampleStart = 0;
    m_SampleStop = 1;
    m_TimeStart = 0.0;
    m_TimeStop = 1.0;
