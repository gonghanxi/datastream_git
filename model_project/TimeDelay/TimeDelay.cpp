...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE(TimeDelay)
{
...
bool TimeDelay::Setup()
{
	...
	if (T < 0.0) {
		POST_ERROR("TimeDelay: T must be >= 0.");
		return false;
	}
	if (N < 0) {
		POST_ERROR("TimeDelay: N must be >= 0.");
		return false;
	}
...
ERESULT TimeDelay::CalculateLatency()
{
	...
	if (Unit == Unit_Time) {
		...
		if (dtIn <= 0.0) {
			POST_ERROR("TimeDelay: input signal must be timed (sample rate > 0).");
			return -1;              
		}
