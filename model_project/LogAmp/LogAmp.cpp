...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE ( LogAmp )
{	
	SET_MODEL_DESCRIPTION("Logarithmic Amplifier");
	SET_MODEL_SYMBOL("SYM_LogAmp");
...
bool LogAmp::Run()
{
	...
	double Mt = At > VL ? 20 * Sensitivity*std::log10(At / VL) + ep : 0;

	output[0] = Mt / At * input[0].complex();
	return true;
}
