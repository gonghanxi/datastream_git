#include "AsyncCommutatorInt_Block.h"

#include <algorithm>
#include <vector>

AsyncCommutatorInt_Block::AsyncCommutatorInt_Block(const std::string& name)
    : Block(name)
    , m_blockSizes(1, 1)
{
    m_blockSizes(0) = 1;
...
void AsyncCommutatorInt_Block::SetDefaultParamters()
{
    m_blockSizes.Resize(1, 1);
    m_blockSizes(0) = 1;
}

void AsyncCommutatorInt_Block::SetParameters()
{
    if (!m_asyncCommutatorInt) {
        return;
    }

    m_asyncCommutatorInt->BlockSizes = m_blockSizes;
}

bool AsyncCommutatorInt_Block::Setup()
{
    Block::Setup();
...
bool AsyncCommutatorInt_Block::Run()
{
    if(IsVariableStepMode()) return TimeDrivenRun();
    return DataStreamRun();
}

bool AsyncCommutatorInt_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);

    m_asyncCommutatorInt = std::make_unique<AsyncCommutatorInt>();

    AddInputPort("input", m_asyncCommutatorInt->input, 1, Block::DataType::INT_BUS);
    AddOutputPort("output", m_asyncCommutatorInt->output, 1, Block::DataType::CIRCULAR_BUFFER_INT);
...
bool AsyncCommutatorInt_Block::ModelSetup()
{
...
bool AsyncCommutatorInt_Block::DataStreamRun()
{
...
bool AsyncCommutatorInt_Block::TimeDrivenRun()
{
    ...
    if (!m_outputQueue.empty()) {
        ...
        qDebug() << "[AsyncCommutator] no data:" << m_lastOutput;
    }
