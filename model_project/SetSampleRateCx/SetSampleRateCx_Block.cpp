#include "SetSampleRateCx_Block.h"
#include <cmath>
...
SetSampleRateCx_Block::SetSampleRateCx_Block(const std::string& name)
	: Block(name)
{
}

void SetSampleRateCx_Block::SetDefaultParamters()
{
	m_sampleRate = getSimu().samplingRate;
}

void SetSampleRateCx_Block::SetParameters(double sampleRate)
{
...
bool SetSampleRateCx_Block::Setup()
{
	Block::Setup();
	return true;
}

bool SetSampleRateCx_Block::Run()
{
	if (!CanProcess()) {
		return false;
	}
...
bool SetSampleRateCx_Block::Initialize()
{
	SetBlockType(Block::BlockType::PROCESSOR);

	m_setSampleRate = std::make_unique<SetSampleRateCx>();

	AddInputPort("input", m_setSampleRate->input, 1, Block::DataType::TIMED_DCOMPLEX);
...
bool SetSampleRateCx_Block::ValidateSampleRate()
{
	if (!(m_sampleRate > 0.0)) {
