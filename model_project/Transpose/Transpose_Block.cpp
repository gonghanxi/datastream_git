#include "Transpose_Block.h"

Transpose_Block::Transpose_Block(const std::string &name)
    :Block(name)
{

}

bool Transpose_Block::Setup()
{
    Block::Setup();
...
bool Transpose_Block::Initialize()
{
    SetBlockType(Block::BlockType::PROCESSOR);
    m_Transpose = std::make_unique<Transpose>();
    ...
    AddInputPort("input", m_Transpose->input, static_cast<size_t>(SamplesInRow * NumberOfRows), DataType::CIRCULAR_BUFFER_DOUBLE);
    AddOutputPort("output", m_Transpose->output, static_cast<size_t>(SamplesInRow * NumberOfRows), DataType::CIRCULAR_BUFFER_DOUBLE);
    return true;
}

bool Transpose_Block::Run()
{
    if(IsVariableStepMode()) return TimeDrivenRun();
    return DataStreamRun();
}

void Transpose_Block::SetParameters()
{
...
void Transpose_Block::SetDefaultParameters()
{
    SamplesInRow = 8;
    NumberOfRows = 8;
}

bool Transpose_Block::ModelSetup()
{
...
bool Transpose_Block::DataStreamRun()
{
    auto inputData = ReadInputData<double>(GetInputPortName(0));
    std::vector<double> outputData(SamplesInRow * NumberOfRows);
...
bool Transpose_Block::TimeDrivenRun()
{
    auto inputData = ReadInputData<double>(GetInputPortName(0));
    ...
    if(m_inputBuffer.size() >= static_cast<size_t>(SamplesInRow * NumberOfRows)) {
        ...
        if (!m_outputQueue.empty()) {
            ...
            qDebug() << "[Transpose_Block] 分发输出:" << m_outputCount
                     << " value:" << outputValue;
            m_inputBuffer.clear();
        }
    }
