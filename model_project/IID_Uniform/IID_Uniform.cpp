...
#ifndef SV_CODE_GEN
DEFINE_MODEL_INTERFACE ( IID_Uniform )
{	
	SET_MODEL_DESCRIPTION("IID Uniform Noise Waveform");
	SET_MODEL_SYMBOL("SYM_IID_Uniform");
...
bool IID_Uniform::Run()
{
	...
	std::uniform_real_distribution<double>	dN(LoLevel, HiLevel);
	output[0] = dN(gen);
	...
	// 单脉冲模式输出
	if (BurstMode == IID_Uniform::Single)
	{
	...
	// 多脉冲模式疏输出
	if (BurstMode == IID_Uniform::Multiple)
	{
